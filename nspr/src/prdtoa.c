/* -*- Mode: C; tab-width: 4 -*-
** Portable safe double precision functions taken from code written by
** By Gay.
**
** Author: Ricardo Jenez
*/
...
PR_cnvtf(char *buf,int bufsz, int prcsn,double fval)
{
	...
	num = PR_dtoa(fval,1,prcsn,&decpt,&sign,&endnum);
	...
	if(decpt > (prcsn+1) || decpt < -(prcsn-1)){
	    *bufp++ = *num++;
	    if(numdigits != 1){
			*bufp++ = '.';
	    }

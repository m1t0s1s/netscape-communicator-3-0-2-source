...
NET_StreamClass *fe_MakeSaveAsStream (int format_out, void *data_obj,
				      URL_Struct *url_struct,
				      MWContext *context);

#define NPX_PLUGIN_PATH "/usr/local/lib/netscape/plugins"
...
void FE_RegisterPlugins()
{
    ...
    char *envPluginPath = getenv("NPX_PLUGIN_PATH"), *pluginPath = NULL;
    ...
    if (!envPluginPath) {
	/* Default plugin search path is:
	 *	/usr/local/lib/netscape/plugins		- NPX_PLUGIN_PATH
	 *	~/.netscape/plugings
	 */
	pluginPath = PR_smprintf("%s:%.900s/.netscape/plugins",
					NPX_PLUGIN_PATH, home);
    }
    else pluginPath = envPluginPath;
    ...
    if (!envPluginPath) XP_FREE(pluginPath);
...
fe_RegisterPluginConverters(void)
{
  ...
  for (n=0; n<list->num; n++) {
	...
	if (md_item) {
	  ...
	  if (md_item->xmode &&
	      !strcasecmp(md_item->xmode, NET_COMMAND_SAVE_BY_NETSCAPE)) {
...
NPPluginFuncs *
FE_LoadPlugin(void *pdesc, NPNetscapeFuncs *funcs)
{
#ifdef X_PLUGINS
...
void FE_UnloadPlugin(void *pdesc)
{
#ifdef X_PLUGINS
    NPX_PlugIn *plugin =  (NPX_PlugIn *) pdesc;
    NPError (*f)(void);

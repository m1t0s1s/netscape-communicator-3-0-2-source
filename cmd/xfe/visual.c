...
fe_ParseVisual (Screen *screen, const char *v)
{
  ...
  else if (!strcasecmp (v, "default"))		  vclass = DEFAULT_VISUAL;
  else if (!strcasecmp (v, "best")) 		  vclass = BEST_VISUAL;
  else if (!strcasecmp (v, "staticgray")) 	  vclass = StaticGray;
  else if (!strcasecmp (v, "staticcolor"))	  vclass = StaticColor;
  else if (!strcasecmp (v, "truecolor"))	  vclass = TrueColor;
  else if (!strcasecmp (v, "grayscale"))	  vclass = GrayScale;
  else if (!strcasecmp (v, "pseudocolor"))	  vclass = PseudoColor;
  else if (!strcasecmp (v, "directcolor"))	  vclass = DirectColor;

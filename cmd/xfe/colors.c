...
struct fe_colormap
{
  ...
  il_colorspace *colorSpace;    /* Image library color conversion state */
};
               ...
               Colormap cmap, Boolean private_p)
{
  ...
  /* Isn't it swell that we have to go through this shit just to
...
*/
int
XFE_SetColormap (MWContext *context,
                 IL_IRGB *map_in_out,
                 int num_requested)
{
  ...
  IL_IRGB sorted_map[256], *map_in, *map_out;
  XColor color;
  int i, j, free_count;
  ...
  if (num_requested > free_count)
    {
     ...
     qsort (color_sort_array, num_requested, sizeof(struct color_distance),
            compare_color_distances);

     /* Reorder the request colors to correspond to the sorted order */
     for (i = 0; i < num_requested; i++)
         sorted_map[i] = map_in[color_sort_array[i].index];
    ...
    for (i = 0, mp = map_in; i < num_requested; i++, mp++) {
      ...
      if (mp->attr & IL_ATTR_TRANSPARENT)
      ...
      else
        {
          ...
          map_out[mp->index].red   = actual_red;
          map_out[mp->index].green = actual_green;
          map_out[mp->index].blue  = actual_blue;
        }
      map_out[mp->index].index = pixel;
...
static void
FindShift (unsigned long mask, int *shiftp, int* bitsp)
{
  int shift = 0;
  int bits = 0;
  while ((mask & 1) == 0) {
...
fe_InitColormap (MWContext *context)
{
  ...
  if (colormap->contexts > 1)
      context->colorSpace = colormap->colorSpace;
  ...
  if (vi_out->depth == 1 || fe_globalData.force_mono_p || broken_visual)
    {
      t.index = 0xff;           /* Index doesn't matter */
      t.red   = 0xff;
      t.green = 0xff;
      t.blue  = 0xff;

      if (colormap->contexts > 1) {
        assert(colormap->colorSpace);
	 ...
	 We will still enlarge these to visual-depth when displaying them. */
      IL_GreyScale (context, 1, &t);
    }
  else if (vi_out->class == StaticGray ||
	   vi_out->class == GrayScale)
    {
      XColor color;
      t.index = color.pixel = CONTEXT_DATA (context)->default_bg_pixel;
      fe_QueryColor (context, &color);
      t.red = color.red >> 8;
      t.green = color.green >> 8;
      t.blue = color.blue >> 8;
      ...
      IL_GreyScale (context, pixmap_depth, &t);
    }
	   ...
	   vi_out->class == DirectColor)
    {
	 ...
	 I suppose it might be possible that some system somewhere
       ...
       */
      FindShift (visual->red_mask, &rs, &rbits);
      FindShift (visual->green_mask, &gs, &gbits);
      FindShift (visual->blue_mask, &bs, &bbits);
      ...
      IL_EnableTrueColor (context, pixmap_depth,
			  rs, gs, bs, rbits, gbits, bbits, &t, False);
    }
  else
    {
      int cube_count, max;
      ...
      if (!warned && !cube_count)
	{
      ...
      if (!warned && cube_count < 48 && 
	  vi_out->class == PseudoColor)
	{
	  char buf [2048];
	  PR_snprintf (buf, sizeof (buf),
			fe_globalData.cube_too_small_message, cube_count);
	  FE_Alert (context, buf);
	  warned = True;
	}
    }

  colormap->colorSpace = context->colorSpace;

colorspace_init_complete:

  if (!fe_ImagesCantWork)
    {
      extern IL_ColorRenderMode fe_pref_string_to_dither_mode (char *s);

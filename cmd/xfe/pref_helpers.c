...
static char* cleanString(char *newbuf )
{
	...
	found = 0;
	i = 0;
	while (!found && strlen(newbuf) > 0 && i < strlen(newbuf))
	  {
		if ( newbuf[i] == '\\' ||
			 newbuf[i] == ';' )
		  {
...
fe_helpers_is_deleted_type (NET_cdataStruct *cd_item)
{
	...
	if ( md_item && md_item->xmode && *md_item->xmode )
		if ( !strcasecmp(md_item->xmode, NET_COMMAND_DELETED))
			return True;
		else return False;
...
fe_helpers_plugin_selected(struct fe_prefs_helpers_data *fep, char *pluginName)
{
	...
	mimetype = XmTextFieldGetString(fep->mime_types_text);
	...
	if (fe_GetMimetypeInfoFromPlugin(pluginName, mimetype, &desc, &ext) >= 0) {
	   ...
	   * plugin says.
       */
	  cur_desc = XmTextFieldGetString(fep->mime_types_desc_text);
	  cur_ext = XmTextFieldGetString(fep->mime_types_suffix_text);
...
fe_helpers_build_handle(struct fe_prefs_helpers_data *fep)
{
	...
	if ( XmToggleButtonGetState(fep->navigator_b) )
	  ...
	  {
		char * text = XmTextFieldGetString(fep->app_text);
		if ( text && *text)
		  {
...
fe_helpers_done_cb(Widget widget, XtPointer closure, XtPointer call_data)
{
	...
	/* Description */
	text = XmTextFieldGetString(fep->mime_types_desc_text);
	if ( text && *text ) {
	  fe_StringTrim(text);
	...
	/* Suffix */
	text = XmTextFieldGetString(fep->mime_types_suffix_text);
	u = XP_STRTOK(text, ",");
	...
	while ((md_item = (NET_mdataStruct *)XP_ListNextObject(infolist)))
	  {
		if ( type && md_item->contenttype &&
			 !strcasecmp(type, md_item->contenttype))
		  {
			new_item = md_item;
			break;
		  }

...
*/
  ...
  XmScrolledWindowSetAreas (scroller, hscroll, vscroll, drawing_area);
...
void
fe_RefreshArea(MWContext* context, int32 x, int32 y, uint32 w, uint32 h)
{
    LO_RefreshArea(context, x, y, w, h);
    
#ifdef EDITOR
    if (EDT_IS_EDITOR(context))
        fe_EditorRefreshArea(context, x, y, w, h);
#endif /*EDITOR*/
}
  ...
  if (horiz_p)
      ...
      if (new_x >= old_x)
	{
	  ...
	  if (CONTEXT_DATA (context)->backdrop_pixmap)
	    fe_ClearArea (context, w - shift, 0, shift, h);
	  if (fe_globalData.fe_guffaw_scroll == 0)
	    fe_RefreshArea (context,
			  new_x + w - sb_w - shift,
			  new_y, shift + sb_w, h);
      ...
      else
	{
	  ...
	  if (CONTEXT_DATA (context)->backdrop_pixmap)
	    fe_ClearArea (context, 0, 0, shift, h);
	  if (fe_globalData.fe_guffaw_scroll == 0)
      ...
      if (new_y >= old_y)
	{
	  ...
	  if (CONTEXT_DATA (context)->backdrop_pixmap)
	    fe_ClearArea (context, 0, h - shift, w, shift);
	  if (fe_globalData.fe_guffaw_scroll == 0)
      ...
      else
	{
	  ...
	  if (CONTEXT_DATA (context)->backdrop_pixmap)
	    fe_ClearArea (context, 0, 0, w, shift);
	  if (fe_globalData.fe_guffaw_scroll == 0)
...
fe_ScrollTo (MWContext *context, unsigned long x, unsigned long y)
{
  if (x != CONTEXT_DATA (context)->document_x)
	...
	x = nx;
      ...
      cb.value = x;
      fe_scroll_cb (CONTEXT_DATA (context)->hscroll, context, &cb);
    }
  if (y != CONTEXT_DATA (context)->document_y)
	...
	y = ny;
      ...
      memset (&cb, 0, sizeof (cb));
      cb.value = y;
      fe_scroll_cb (CONTEXT_DATA (context)->vscroll, context, &cb);
    }
}
  ...
  if (fe_globalData.fe_guffaw_scroll)
      ...
      else
	{
	  ...
	  if (CONTEXT_DATA (context)->held_expose == FALSE)
		...
		new_height = 1;
	      if (CONTEXT_DATA (context)->backdrop_pixmap)
		fe_ClearArea (context, new_x, new_y, new_width, new_height);
	      fe_RefreshArea (context,
    ...
    {
      if (CONTEXT_DATA (context)->backdrop_pixmap)
	fe_ClearArea (context, ee->x, ee->y, ee->width, ee->height);
      fe_RefreshArea (context,
...
void
XFE_SetDocPosition (MWContext *context, int iLocation,
		   int32 iWidth, int32 iLength)
{
  unsigned long old_x = CONTEXT_DATA (context)->document_x;
  unsigned long old_y = CONTEXT_DATA (context)->document_y;

  fe_SetDocPosition (context, iWidth, iLength);

  /* If the document has been scrolled, then the whole backdrop image
     needs to be repainted - else the border regions and any dead space
     within the document (of which there really should be none) will
     show up with the image shifted.

     This must only be done when layout asks us to scroll to set the
     initial position of the document.  It should not be done on subsequent
     scrolls by the user, since fe_ScrollTo will take care of moving the
     backdrop image for us.  This may not be the right place for this code.
   */
  if ((old_x != CONTEXT_DATA (context)->document_x ||
       old_y != CONTEXT_DATA (context)->document_y) &&
...
fe_WindowGravityWorks (Widget toplevel, Widget widget)
{
  ...
  else if (str && (!strcasecmp (str, "partial") || !strcasecmp (str, "kindof")))
    {
      return 2;
    }
  else if (str && (!strcasecmp (str, "no") || !strcasecmp (str, "false")))
    {
      return 0;
    }
  else

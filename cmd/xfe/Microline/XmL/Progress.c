...
Cardinal *narg;
	...
	p->progress.gc = XCreateGC(dpy, root, GCFont, &values);
...
XRectangle *rect;
	...
	str = XmStringCreate(c, XmSTRING_DEFAULT_CHARSET);
...
Cardinal *narg;
	...
	if (p->progress.completeValue < 1)
		{
		XmLWarning(newW, "SetValues() - complete value cannot be < 1");
		p->progress.completeValue = 1;
		}
	if (p->progress.numBoxes < 1)
		{
		XmLWarning(newW, "SetValues() - number of boxes cannot be < 1");
		p->progress.numBoxes = 1;
		}
	if (p->progress.value < 0)
		{
		XmLWarning(newW, "SetValues() - value cannot be < 0");
		p->progress.value = 0;
		}
	if (p->progress.value > p->progress.completeValue)
		{
		XmLWarning(newW, "SetValues() - value cannot be > completeValue");
...
XtPointer *data;
	...
	if (!valid)
		{
		XtDisplayStringConversionWarning(dpy, from, "XmRMeterStyle");
		toVal->size = 0;
		toVal->addr = 0;
		return False;
		}

...
/* Don't use async DNS in my Solaris debug builds, because the only time 
   I build on Solaris is when I'm gonna use Purify, and Purify hates fork().
 */
#if defined(DEBUG) && defined(DEBUG_jwz)
# if defined(__sun) && defined(__svr4__)
#  undef UNIX_ASYNC_DNS
# endif
#endif
  ...
  for (obj = queue; obj; obj = obj->next)
      ...
      if (socket == obj->socket)
	{
	  ...
	  if (!!strcasecomp(host, obj->name))
	    {
	      LOG("FE_StartAsyncDNSLookup (2.5)", ("netlib is fucked!!\n"));
	      /* XP_ASSERT(0); */
	      free_lookup(obj);
	      obj = 0;

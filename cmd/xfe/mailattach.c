...
fe_locationOk_cb (Widget widget, XtPointer closure, XtPointer call_data)
{
  ...
  char *url = XmTextFieldGetString(mad->location_text);
...
/*  XtSetArg (av[ac], XmNtransientFor, parent); ac++; */
...

  {
    ...
    XmTextFieldSetString (location_text, (char *) (str ? str : ""));
  }
...
/*
  ptr = MSG_GetAttachmentString(mad->comppane);
*/
  ptr = MSG_GetCompHeader(mad->comppane, MSG_ATTACHMENTS_HEADER_MASK);
  XmTextFieldSetString(CONTEXT_DATA(mad->context)->mcAttachments,
		       ptr ? ptr : "");
  if (ptr) XP_FREE(ptr);
...
extern 
void
fe_MailComposeWin_ActivateFolder(int pos);
...
fe_mailto_attach_dialog(MWContext* context)
{
  ...
  XP_ASSERT(context->type == MWContextMessageComposition);
  fe_MailComposeWin_ActivateFolder(1);
		   ...
		   fe_dnd_Source* source, XEvent* event)
{
  ...
  XmTextFieldSetString(CONTEXT_DATA(compose_context)->mcAttachments,
		       s ? s : "");
  if (s) XP_FREE(s);
}

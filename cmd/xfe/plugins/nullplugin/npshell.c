...
NPP_Destroy(NPP instance, NPSavedData** save)
{
	...
	if (This->dialog) {
	  /* Remove any destroy callback as that could use This after we
	   * freed it.
	   */
	  XtRemoveAllCallbacks(This->dialog, XmNdestroyCallback);
	  This->dialog = 0;
	}
...
NPP_SetWindow(NPP instance, NPWindow* window)
{
	...
	if ((Window)window->window == This->window) {
	  /* This could happen because of a resize of the plugin area (or)
	   * simply the navigator making some extra SetWindow() calls.
	   *
	   * Null plugin doesn't resize.
	   */
	  return NPERR_NO_ERROR;
	}
...

	{
	    ...
	    if (This->dialog) {
	      /* This can happen when we have a plugin in a table and the
	       * table cell is being relaid out.
	       */

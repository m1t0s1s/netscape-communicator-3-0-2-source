...
#include "ntypes.h"
#include "shistele.h"
#include "xp_sock.h"  /* for NETSOCK etc... */
...
(*MKStreamWriteReadyFunc) (void *data_object);
...
typedef void 
(*MKStreamCompleteFunc) (void *data_object);

typedef void 
(*MKStreamAbortFunc) (void *data_object, int status);
...
#define APPLICATION_NS_JAVASCRIPT_AUTOCONFIG		"application/x-ns-javascript-autoconfig"
 ...
 * bit flags in "wanted"
 *
 * mallocs a returned string that must be freed
 */
 ...
 */
extern int NET_AddLOSubmitDataToURLStruct(LO_FormSubmitData * sub_data, URL_Struct * url_struct);
...
extern void NET_SetCookiePrefs(NET_CookiePrefsEnum pref_enum);
 ...
 */
extern NET_StreamClass *
NET_CloneWysiwygCacheFile(MWContext *context, URL_Struct *URL_s, uint32 nbytes);
 ...
 */
extern int 
FE_StartAsyncDNSLookup(MWContext *context, char *host_and_port,
					   void **hoststruct_ptr_return, int sock);
 ...
 * returns -1 on error.
 */
extern int NET_InterruptSocket (NETSOCK socket);
 ...
 */
extern void RealNET_RegisterContentTypeConverter (char          * format_in,
                                              FO_Present_Types format_out,
                                              void          * data_obj,
                                              NET_Converter * converter_func);
/*  This function found in the windows front end.
 */
extern BOOL PMS_RegisterContentTypeConverter (char          * format_in,
                                              FO_Present_Types format_out,
                                              void          * data_obj,
                                              NET_Converter * converter_func,
                                              BOOL          bAutomated);
 ...
 */
#ifdef OLD_MIME
extern void NET_ScanForURLS(MWContext *context, char * input, char *output,
					int output_size, XP_Bool urls_only_p);
#else  /* !OLD_MIME */
extern int NET_ScanForURLs(const char *input, int32 input_size,

...
static void bm_SortSelected(MWContext* context);
static void bm_SyncCount(MWContext* context);
static void bm_RemoveChildFromHeader(MWContext* context, BM_Entry* parent,
									 BM_Entry* child);
...
bm_end_batch(MWContext* context)
{
  ...
  if (f->undo) UNDO_EndBatch(f->undo);
}
...
bm_refresh(MWContext* context, int32 first, int32 last)
{
  ...
  if (first < 1 || first > last) {
	/* Screw it.  Something bogus got passed in; just repaint everything to
	   be safe. */
	first = 1;
	last = BM_LAST_CELL;
  }
...
bm_entry_changed_2(MWContext* context, BM_Entry* entry)
{
  ...
  if (context->type == MWContextBookmarks || entry->parent == NULL) {
	...
	/* Changing the entry might have screwed up the sorting order.  Better
  ...
  if (context->type == MWContextBookmarks && f && BM_ISHEADER(entry)) {
	if (f->undo) {
	  ...
	  if (!info) {
		...
		info->isadd = isadd;
		UNDO_LogEvent(f->undo, bm_setheader_undo, bm_setheader_freeit, info);
	  }
	}
...
/* returns the child url entry of parent whose address is the same as
   url_address */
PRIVATE BM_Entry*
bm_FindItemStub(BM_Entry* parent, char* url_address)
{
  BM_Entry*	child;
  ...
  for (child = parent->d.header.children; child; child = child->next) {
	...
	if (child->type == BM_TYPE_HEADER) {
	  BM_Entry* rv;
	  rv = bm_FindItemStub(child, url_address);
	  if (rv) return rv;
	}
  }
  return NULL;
...
bm_LogDeleteChild(MWContext* context, BM_Entry* parent, BM_Entry* child)
{
  ...
  if (!info) {
	...
	info->parent = parent;
	info->child = child;
	UNDO_LogEvent(f->undo, bm_delete_child_doit, bm_simple_freeit, info);
  }
}
...
static void
bm_RemoveChildFromHeader(MWContext* context, BM_Entry* parent, BM_Entry* child)
{
  BM_Frame* f = GETFRAME(context);
  BM_Entry* previous;
  ...
  if (context) {
	...
	if (f->undo) {
	  ...
	  if (!info) {
		...
		info->child = child;
		UNDO_LogEvent(f->undo, bm_add_child_doit, bm_simple_freeit, info);
	  }
	}
  }
  child->parent = NULL;
...
bm_addition_date(char* buffer)
{
	...
	if ((ptr = strcasestr(buffer, "ADD_DATE=\"")))
	{
			...
			const char* relative_url)
{
  ...
  if (endQuote) {
	...
	*endQuote = '\0';
	...
	url = NET_MakeAbsoluteURL((char* /*FUCK*/)relative_url, parseString);
	if (url) {
...
static XP_Bool
bm_StringMatches(BM_FindInfo* findInfo, const char* str) {
  char* ptr;
  if (!str) return FALSE;
  if (findInfo->matchCase) {
	ptr = strstr(str, findInfo->textToFind);
  } else {
	ptr = strcasestr(str, findInfo->textToFind);
  }
  if (!ptr) return FALSE;
...
static XP_Bool
bm_IsMatch(BM_Entry* entry, BM_FindInfo* findInfo)
{
  if (BM_ISALIAS(entry)) entry = entry->d.alias.original;
  if (findInfo->checkNickname) {
	if (bm_StringMatches(findInfo, entry->nickname)) return TRUE;
  }
  if (findInfo->checkName) {
	if (bm_StringMatches(findInfo, BM_GetName(entry))) return TRUE;
  }
  if (findInfo->checkLocation && BM_ISURL(entry)) {
	if (bm_StringMatches(findInfo, entry->d.url.address)) return TRUE;
  }
  if (findInfo->checkDescription) {
	if (bm_StringMatches(findInfo, entry->description)) return TRUE;
  }
  return FALSE;
}
...
bm_DoFindBookmark_1(MWContext* context, BM_Entry* at, BM_FindInfo* findInfo) {
  ...
  if (!at) return NULL;
  do {
	if (bm_IsMatch(at, findInfo)) return at;
...
bm_write_ok(const char* str, int length, XP_File fp)
{
  ...
  if (XP_FileWrite(str, length, fp) < length) return -1;
  return 0;
}
...
BM_FreeEntry(MWContext* context, BM_Entry* entry)
{
  ...
  if (f->undo) {
	...
	if (!info) {
	  ...
	  info->entry = entry;
	  UNDO_LogEvent(f->undo, bm_cancel_free, bm_free_freeit, info);
	}
  } else {
	bm_ReallyFreeEntry(entry);
  }
...
#ifdef NOTFORNOW				/* This cleanup code can be slow and
...
BM_UpdateBookmarksTime(URL_Struct* URL_s, BM_Date cur_time)
{
  ...
  for (context = ContextList ; context ; context = f->next) {
	...
	hs = bm_FindItemStub(BM_GetRoot(context), URL_s->address);

	if (hs) {
	  int32 oldstate = BM_GetChangedState(hs);
	  hs->d.url.last_visit = cur_time;
	  if (hs->d.url.last_modified == 0) {
		/* Well, this current visitation is a good enough estimate for
		   the modification time. */
		hs->d.url.last_modified = cur_time;
	  }
	  ...
	  if (BM_GetChangedState(hs) != oldstate) {
		bm_entry_changed(context, hs);
	  }
	}
  }
}
			...
			if (bm)
			{
				if (fold)
				{
					if (firstChangedCell != 0)
						bm_ClearSelection(context, bm, FALSE, &count);
...
#ifndef DEBUG_jwz /* I'm tired of seeing this */
		FE_Alert(context,
				 XP_GetString(context->type == MWContextAddressBook ?
  ...
  if (bLongFormat) {
	while (lBytesEaten < lTotalLen) {
	  ...
	  case 12345:
		...
		break;
	  default:
		/* fuck bogus type.  Who knows whats going on.  Just quit and
		   get out */
		goto GETOUT;
...
static void
bm_SortSelected_1(MWContext* context, BM_Entry* header)
{
  BM_Frame* f = GETFRAME(context);
  BM_Entry** list = NULL;
  int numlist;
  ...
  if (list) {
	if (numlist > 1) {
	  ...
	  qsort(list, numlist, sizeof(BM_Entry*), bm_comparenames);
	  for (i=0 ; i<numlist ; i++) {
		if (previous) {
  ...
  for (entry = header->d.header.children ; entry ; entry = entry->next) {
	if (BM_ISHEADER(entry)) bm_SortSelected_1(context, entry);
  }
}
...
static void
bm_SortSelected(MWContext* context)
{
  BM_Frame* f = GETFRAME(context);
  if (f->gSelectionCount < 0) bm_SyncSelection(context);
  bm_SortSelected_1(context, BM_GetRoot(context));
  bm_refresh(context, 1, BM_LAST_CELL);
}
...
bm_ComposeMessageToSelected(MWContext* context)
{
  ...
  buf = tmp;
  if (!buf) return;
  tmp = XP_Cat("mailto:?to=", buf, (char*)/*StupidWin16*/ NULL);
  XP_FREE(buf);
  buf = tmp;
  if (!buf) return;

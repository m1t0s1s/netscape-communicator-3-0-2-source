 ...
 *
 *    $Id: jpeg.c,v 1.79 1996/05/24 19:48:15 fur Exp $
 */
...
typedef enum {
	...
	JPEG_NEXT_PROGRESSIVE_SCAN_OUTPUT,    /* Start of progressive scan */
...
/* Delay from decode to display of first scan. */
#define JPEG_TIMEOUT_INITIAL_DELAY   250

/*
 * Delays for subsequent scans are scaled to the area of the *displayed*
 * image.  The following duration is for a 512-pixel square image
 */
#define JPEG_TIMEOUT_DELAY          1000

#define JPEG_OUTPUT_CHUNK_SIZE     25000

static void
jpeg_timeout_callback(void *closure)
{
	jpeg_struct *js = (jpeg_struct *)closure;
    js->allowed_output_passes++;
    js->timeout = NULL;
}
	...
	while (! input_exhausted) {
		...
		case JPEG_HEADER:
			if (jpeg_read_header(jd, TRUE) != JPEG_SUSPENDED) {
				im->map = 0;
				im->colors = ic->cs->default_map_size;
                il_setup_color_space_converter(ic);

				if ((status = il_size(ic, jd->image_width, jd->image_height))) {
					XP_TRACE(("il:jpeg: MEM il_size"));
					return status;
				}

                js->rows_per_chunk = JPEG_OUTPUT_CHUNK_SIZE / im->widthBytes;
        ...
        case JPEG_START_DECOMPRESS:
            if (jpeg_start_decompress(jd)) {
                ...
                if (jd->buffered_image) {
                    js->state = JPEG_NEXT_PROGRESSIVE_SCAN_OUTPUT;
                } else {
		...
		case JPEG_NEXT_PROGRESSIVE_SCAN_OUTPUT:
            ...
            if (js->timeout == NULL) {
                ...
                if (jd->input_scan_number == 1)
                    ...
                    /*
                     * Scale time based on picture size.
                     * Be careful to avoid overflowing 32-bit int.
                     */
                    delay = (((delay * im->width) >> 9) * im->height) >> 9;
                }
            ...
            /* Finish display of any previously output scans. */
/*            if (js->completed_output_passes > 0)
...
*/
		...
		case JPEG_DECOMPRESS_PROGRESSIVE:
             ...
             */
            if (! (js->completed_output_passes < js->allowed_output_passes))
                break;

            output_jpeg_scanlines(ic, js->rows_per_chunk);

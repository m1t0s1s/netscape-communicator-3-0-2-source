...
NPError fe_RegisterPlugin(char* pPluginFilespec)
{
	...
	NPL_RegisterPluginFile(strName, pNPMgtBlk->pPluginFilename, strDescription, pNPMgtBlk);
...
NPPluginFuncs* FE_LoadPlugin(void* pluginType, NPNetscapeFuncs* pNavigatorFuncs)
{
    if(pluginType == NULL)
        return NULL;
    ...
    //pNPMgtBlock->hDLLInstance = ::LoadLibrary(pNPMgtBlock->pPluginFilename);

#if 0
#ifdef _WIN32
    if(pNPMgtBlock->hDLLInstance == NULL) {
        DWORD err = ::GetLastError();
#else
    if(pNPMgtBlock->hDLLInstance < HINSTANCE_ERROR) {
#endif
#endif
    ...
    NP_GETENTRYPOINTS getentrypoints =
        ...
        //(NP_GETENTRYPOINTS)::GetProcAddress(pNPMgtBlock->hDLLInstance,
        //                                   "NP_GetEntryPoints");
    if(getentrypoints == NULL)
    {
		PR_UnloadLibrary(pNPMgtBlock->pLibrary);
        //::FreeLibrary(pNPMgtBlock->hDLLInstance);
        wfe_PopPath(pPathSave);  // restore the path
        return NULL;
    }
    ...
    if(pNPMgtBlock->uRefCount == 0) {
        ...
        // name
        NP_PLUGININIT npinit =
			(NP_PLUGININIT)PR_FindSymbol("NP_Initialize", pNPMgtBlock->pLibrary);
            //(NP_PLUGININIT)::GetProcAddress(pNPMgtBlock->hDLLInstance,
            //                                "NP_Initialize");
        if(!npinit) {
            npinit =
    ...
    // can't pop the path until lame plugins have been completely loaded/init'd
    wfe_PopPath(pPathSave);  // restore the path
...
void FE_UnloadPlugin(void* pluginType)
{
    ...
    if(pNPMgtBlk->uRefCount == 0) {
        ...
        NP_PLUGINSHUTDOWN npshutdown =
            ...
            //(NP_PLUGINSHUTDOWN)::GetProcAddress(pNPMgtBlk->hDLLInstance,
            //                                    "NP_Shutdown");
        if(!npshutdown) {
            npshutdown =
    ...
    pNPMgtBlk->pLibrary = NULL;
    //::FreeLibrary(pNPMgtBlk->hDLLInstance);
    //pNPMgtBlk->hDLLInstance = NULL;
...
void FE_ShowScrollBars(MWContext* pContext, XP_Bool show)
{
	if(ABSTRACTCX(pContext)->IsWindowContext())	{
		...
		pWinCX->ShowScrollBarCX(SB_BOTH, show);
	}
}
...
#if 0
void FE_FreeEmbedSessionData(MWContext* pContext, NPEmbeddedApp* pEmbeddedApp)
{
    if(NPL_EmbedDelete(pContext, pEmbeddedApp) == TRUE) {

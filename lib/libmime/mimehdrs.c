...
extern int MK_MIMEHTML_DISP_SUPERSEDES;
...
#ifdef DEBUG_terry
#undef MimeHeaders_write

int MimeHeaders_write(MimeDisplayOptions *opt, char *data, int32 length)
{
	static XP_File fid = 0;
	if (!fid) {
		fid = XP_FileOpen("C:\\temp\\twtemp", xpTemporary, XP_FILE_WRITE);
	}
	XP_FileWrite(data, length, fid);
	XP_FileFlush(fid);
	return MimeOptions_write(opt, data, length, TRUE);
}
...
MimeHeaders_localize_header_name(const char *name, MimeDisplayOptions *opt)
{
  ...
  int display_key =
   (!strcasecomp(name,HEADER_BCC)			 ? MK_MIMEHTML_DISP_BCC :
	...
	!strcasecomp(name,HEADER_SUPERSEDES)	 ? MK_MIMEHTML_DISP_SUPERSEDES :
...
#ifndef AKBAR
  {
	int16 output_csid = INTL_CharSetNameToID((opt->override_charset
											  ? opt->override_charset
											  : opt->default_charset));
...
{
  int status = 0;
  XP_Bool bold_p = FALSE; /* DEBUG_jwz */
  char *contents = MimeHeaders_get (hdrs, name, FALSE, all_p);
  if (!contents) return 0;

#ifdef DEBUG_jwz
  if (!strcasecomp(name, HEADER_X_DIAGNOSTIC) ||
      !strcasecomp(name, HEADER_X_COMMAND) ||
      !strcasecomp(name, HEADER_X_PROCESSED))
    bold_p = TRUE;
#endif /* DEBUG_jwz */

  status = MimeHeaders_write_random_header_1 (hdrs, name, contents, opt,
                                              bold_p);
  XP_FREE(contents);
  return status;
}
...
#ifndef DEBUG_jwz /* don't care about this one */
  HEADER_RESENT_DATE,
#endif
...
#ifdef DEBUG_jwz			/* jwz finds these useful for debugging... */
 /* HEADER_X_MAILER,*/
 /* HEADER_X_NEWSREADER, */
  ...
  for (rest = mime_interesting_headers; *rest; rest++)
	{
	   ...
	   */
			   ...
			   !strcasecomp(name, HEADER_RESENT_MESSAGE_ID) ||
			   !strcasecomp(name, HEADER_SUPERSEDES))
		{
...
#ifdef DEBUG_jwz
      /* Multiple instances of these headers are appended, for display.
       */
      else if (!strcasecomp(name, HEADER_X_DIAGNOSTIC) ||
               !strcasecomp(name, HEADER_X_COMMAND) ||
               !strcasecomp(name, HEADER_X_PROCESSED))
        {
		  status = MimeHeaders_write_random_header (hdrs, name, TRUE, opt);
        }
#endif /* DEBUG_jwz */
...
#else
# define terry_has_lost_his_mind_and_also_macs_suck 900
#endif
  ...
  status = MimeHeaders_grow_obuffer (hdrs,
									 ...
									 100
								 + terry_has_lost_his_mind_and_also_macs_suck
									 );
  if (status < 0) goto FAIL;
...
# endif /* !AKBAR */
  ...
  XP_SPRINTF(out, "\
<TR><TD>\
<TABLE BORDER=0 BGCOLOR=\"#009999\" cellpadding=0 width=100%%><TR><TD>\
<TABLE BORDER=0 BGCOLOR=\"#FFFFFF\" width=100%%><TR><TD>\
...
#ifdef EGREGIOUS_HEADERS
  out = hdrs->obuffer;
  XP_STRCPY(out, "</TABLE></td></tr></TABLE></td></tr></TABLE></td></tr>");
  status = MimeHeaders_write(opt, hdrs->obuffer, XP_STRLEN(hdrs->obuffer));
  if (status < 0) goto FAIL;
#endif /* EGREGIOUS_HEADERS */
...
MimeHeaders_write_citation_headers (MimeHeaders *hdrs, MimeDisplayOptions *opt)
{
  ...
  if (opt->nice_html_only_p) {
	  char ptr[] = "<BLOCKQUOTE TYPE=CITE>" LINEBREAK;
	...
	if (!opt->nice_html_only_p && opt->fancy_links_p) {
		if (opt->attachment_icon_layer_id == 0) {
			...
			XP_SPRINTF(hdrs->obuffer + XP_STRLEN(hdrs->obuffer),
					   "</TD><TD VALIGN=TOP><LAYER name=noattach-%ld>"
					   "</LAYER><ILAYER name=attach-%ld visibility=hide>"
...
# define PUT_STRING(S) do { \
  ...
  PUT_STRING ("<CENTER><TABLE CELLPADDING=8 CELLSPACING=1 BORDER=1>"
			  "<TR><TD NOWRAP>");
...
#if 0	/* This is useless technobabble.  Skip it. */
	  ...
	  if (!desc)
		{
		  ...
		  /* If there's an X-Sun-Data-Description, but it's the same as the
			 X-Sun-Data-Type, don't show it.  MailTool sucks!!
		   */
		  if (desc)
			{

					   ...
					   const char *buffer, int32 size)
{
	...
	{
		char firstLine[256];
		XP_SPRINTF(firstLine, "begin 644 uu%06d\015\012", sUueCounter++);
...
MimeEncoderDestroy (MimeEncoderData *data, XP_Bool abort_p)
{
  ...
  if (!abort_p &&
	  data->in_buffer_count > 0)
	{
	  ...
	  char *out = buf;
	  int j;
	  uint32 n = data->in_buffer[0] << 16;
	  if (data->in_buffer_count > 1)
		n = n | (data->in_buffer[1] << 8);
...
MimeEncoderData *
MimeUUEncoderInit (int (*output_fn) (const char *, int32, void *),
					void *closure)
{
  return mime_encoder_init (mime_uuencode, output_fn, closure);
}

...
#endif /* !AKBAR */
#include "secnav.h"	/* for SECNAV_GetPWKey */
#include "secpkcs7.h"	/* for SEC_PKCS7ContentInfo */
#include "prefapi.h"
...
mime_reformat_date(const char *date, void *stream_closure)
{
  ...
  s = MSG_FormatDate(
#ifndef AKBAR
					 MSG_FindPane(context, MSG_ANYPANE),
					 /* ###tw   MSG_FindPane() is a total hack */
#else  /* AKBAR */
					 context,
#endif /* AKBAR */
					 t);
					  ...
					  void *stream_closure)
{
  ...
  else								  /* damn, need to copy it first */
	{
  ...
  converted = IntlDecodeMimePartIIStr(line, msd->context->win_csid, FALSE);
...
#endif
  return msd->stream->put_block (msd->stream->data_object, buf, size);
}

static int
mime_reset_html_state_fn (void *stream_closure, XP_Bool abort_p)
{
#if 1
...
#endif /* 0 */
  return mime_output_fn(random_close_tags, XP_STRLEN(random_close_tags),
						stream_closure);
}


static int
mime_display_stream_write (void *stream_closure,
						   const char* buf,
						   int32 size)
{
  struct mime_stream_data *msd = (struct mime_stream_data *) stream_closure;
...
static unsigned int
mime_display_stream_write_ready (void *stream_closure)
{
  struct mime_stream_data *msd = (struct mime_stream_data *) stream_closure;
  if (msd->stream)
    return msd->stream->is_write_ready (msd->stream->data_object);
  else
...
static void
mime_display_stream_complete (void *stream_closure)
{
  struct mime_stream_data *msd = (struct mime_stream_data *) stream_closure;
  MimeObject *obj = (msd ? msd->obj : 0);
  if (obj)
  ...
  if (msd->stream)
	{
	  ...
	  /* Hold your breath, there, Spanky.  This is how we notify compose.c
	  ...
	  msd->stream->complete (msd->stream->data_object);
	  XP_FREE (msd->stream);
	}
#ifdef DEBUG_terry
...
static void
mime_display_stream_abort (void *stream_closure, int status)
{
  struct mime_stream_data *msd = (struct mime_stream_data *) stream_closure;
  MimeObject *obj = (msd ? msd->obj : 0);
  if (obj)
  ...
  if (msd->stream)
	{
	  msd->stream->abort (msd->stream->data_object, status);
	  XP_FREE (msd->stream);
	}
  XP_FREE(msd);
}
...
static unsigned int
mime_insert_html_write_ready(void* data_object)
{
  return MAX_WRITE_READY;
}

static int
mime_insert_html_put_block(void* data_object, const char* str, int32 length)
{
  struct mime_stream_data* msd = (struct mime_stream_data*) data_object;
  char* s = (char*) str;
  char c = s[length];
  XP_ASSERT(msd);
  if (!msd) return -1;
  ...
  EDT_PasteQuoteINTL(msd->context, s, msd->lastcsid);
  if (c) {
	s[length] = c;
  }
  return 0;
...
static void
mime_insert_html_complete(void* data_object)
{
  struct mime_stream_data* msd = (struct mime_stream_data*) data_object;
  XP_ASSERT(msd);
  if (!msd) return;
  EDT_PasteQuote(msd->context, "</BLOCKQUOTE>" LINEBREAK);
  EDT_PasteQuoteEnd(msd->context);
}

static void
mime_insert_html_abort(void* data_object, int status)
{
  mime_insert_html_complete(data_object);
}
								  ...
								  void *stream_closure)
{
  struct mime_stream_data *msd = (struct mime_stream_data *) stream_closure;
  int status;
  uint16 old_csid = msd->context->doc_csid;
					   ...
					   URL_Struct *url, MWContext *context)
{
  ...
  msd->options->reset_html_state_fn   = mime_reset_html_state_fn;
#ifndef AKBAR
  if (format_out == FO_QUOTE_HTML_MESSAGE) {
...
#ifdef XP_WIN
	  /* For windows, printing is the same as going to the screen. */
...
mime_image_end(void *image_closure, int status)
{
  ...
  if (mid->istream)
	{
	  if (status < 0)
		mid->istream->abort(mid->istream->data_object, status);
	  else
		mid->istream->complete(mid->istream->data_object);
	  XP_FREE (mid->istream);
	}
  if (mid->url_struct)
	NET_FreeURLStruct (mid->url_struct);
...
mime_image_write_buffer(char *buf, int32 size, void *image_closure)
{
  ...
  return mid->istream->put_block(mid->istream->data_object, buf, size);
}
...
static void
mime_get_security_info_1(MWContext *context, const char *url,
						 SEC_PKCS7ContentInfo **encrypted_content_info_return,
						 SEC_PKCS7ContentInfo **signed_content_info_return,
						 char **sender_email_addr_return,
						 int32 *decode_error_return,
						 int32 *verify_error_return)
{
  char *addr;
  *encrypted_content_info_return = 0;
  *signed_content_info_return = 0;
  *decode_error_return = 0;
  *verify_error_return = 0;
  if (sender_email_addr_return)
	*sender_email_addr_return = 0;

  /* If there's no URL, or if this window hasn't displayed a MIME object,
	 give up. */
  if (!url ||
	  !context->mime_data ||
	  !context->mime_data->last_parsed_url)
  ...
  /* If the URLs don't match, give up.  (ignore any part-numbers
	 when comparing.)
   */
  {
	char *url1 = mime_set_url_part(url, "", FALSE);
	char *url2 = mime_set_url_part(context->mime_data->last_parsed_url,
								   "", FALSE);
	XP_Bool match = (url1 && url2 && !XP_STRCMP(url1, url2));
	FREEIF(url1);
	FREEIF(url2);
	if (!match) return;
  }

  addr = mime_extract_relative_part_address(context, url);
  mime_find_security_info_of_part(addr,
								  context->mime_data->last_parsed_object,
								  (void **) encrypted_content_info_return,
								  (void **) signed_content_info_return,
								  sender_email_addr_return,
								  decode_error_return,
								  verify_error_return);
  XP_FREE(addr);
}
...
/* Given a URL which indicates a PKCS7 MIME object, returns the corresponding
   security info (or 0).  Returned values:
 ...
 */
void
MimeGetSecurityInfo(MWContext *context, const char *url,
					SEC_PKCS7ContentInfo **encrypted_content_info_return,
					SEC_PKCS7ContentInfo **signed_content_info_return,
					char **sender_email_addr_return,
					int32 *decode_error_return,
					int32 *verify_error_return)
{
  ...
  *signed_content_info_return = 0;
  ...
  if (sender_email_addr_return) *sender_email_addr_return = 0;

  mime_get_security_info_1 (context, url,
							encrypted_content_info_return,
							signed_content_info_return,
							sender_email_addr_return,
							decode_error_return, verify_error_return);
  if (*encrypted_content_info_return || *signed_content_info_return)
	return;
  else
	{
	  /* Ok, that didn't work, let's go look in all the other contexts! */
		  ...
		  if (cx2 != context)
			{
			  mime_get_security_info_1 (context, url,
										...
										decode_error_return,
										verify_error_return);
			  if (*encrypted_content_info_return ||
				  *signed_content_info_return)
				return;
			}
		}
...
{
  XP_ASSERT(context);
  if (!context || !context->mime_data) return;
 ...
 */
void
MIME_GetMessageCryptoState(MWContext *context,
                           XP_Bool *signed_p,
                           XP_Bool *encrypted_p,
                           XP_Bool *signed_ok,
                           XP_Bool *encrypted_ok)
{
  MimeObject *obj;
  XP_ASSERT(signed_p || encrypted_p || signed_ok || encrypted_ok);
  if (signed_p) *signed_p = FALSE;
  if (encrypted_p) *encrypted_p = FALSE;
  if (signed_ok) *signed_ok = FALSE;
  if (encrypted_ok) *encrypted_ok = FALSE;
  ...
  obj = context->mime_data->last_parsed_object;

  mime_get_crypto_state(obj, signed_p, encrypted_p, signed_ok, encrypted_ok);
#endif /* HAVE_MIME_DATA_SLOT */
...
mime_richtext_stream_fn (char *buf, int32 size, void *closure)
{
  ...
  return mrd->stream->put_block (mrd->stream->data_object, buf, size);
}
...
static int
mime_richtext_write (void *closure, const char* buf, int32 size)
{
  struct mime_richtext_data *data = (struct mime_richtext_data *) closure;
...
static unsigned int
mime_richtext_write_ready (void *closure)
{
  struct mime_richtext_data *data = (struct mime_richtext_data *) closure;
  if (data->stream)
    return ((*data->stream->is_write_ready)
			(data->stream->data_object));
  else
    return (MAX_WRITE_READY);
}

static void
mime_richtext_complete (void *closure)
{
  struct mime_richtext_data *mrd = (struct mime_richtext_data *) closure;
  if (!mrd) return;
  FREEIF(mrd->obuffer);
  if (mrd->stream)
	{
	  mrd->stream->complete (mrd->stream->data_object);
	  XP_FREE (mrd->stream);
	}
  XP_FREE(mrd);
}
...
static void
mime_richtext_abort (void *closure, int status)
{
  struct mime_richtext_data *mrd = (struct mime_richtext_data *) closure;
  if (!mrd) return;
  FREEIF(mrd->obuffer);
  if (mrd->stream)
	{
	  mrd->stream->abort (mrd->stream->data_object, status);
	  XP_FREE (mrd->stream);
	}
  XP_FREE(mrd);
}
...
static int
mime_vcard_write ( void *stream_closure,
			  const char *buf,
			  int32 size )
{
  struct mime_vcard_data *vcd = (struct mime_vcard_data *) stream_closure;
  XP_ASSERT ( vcd );
...
static unsigned int
mime_vcard_write_ready ( void *stream_closure )
{
  struct mime_vcard_data *vcd = (struct mime_vcard_data *) stream_closure;
  XP_ASSERT (vcd);
  ...
  if (vcd->stream)
    return vcd->stream->is_write_ready ( vcd->stream->data_object );
  else
    return MAX_WRITE_READY;
}

static void
mime_vcard_complete ( void *stream_closure )
{
  struct mime_vcard_data *vcd = (struct mime_vcard_data *) stream_closure;
  ...
  if (vcd->obj) {
    ...
    if (vcd->stream) {
      vcd->stream->complete (vcd->stream->data_object);
      XP_FREE( vcd->stream );
      vcd->stream = 0;
    }
  }
...
static void
mime_vcard_abort ( void *stream_closure, int status )
{
  struct mime_vcard_data *vcd = (struct mime_vcard_data *) stream_closure;
  ...
  if (vcd->obj) {
	  ...
	  if (vcd->stream) {
		  vcd->stream->abort (vcd->stream->data_object, status);
		  XP_FREE( vcd->stream );
		  vcd->stream = 0;
	  }
  }

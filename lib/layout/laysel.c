...
void
lo_SetInsertPoint(MWContext *context, lo_TopState *top_state, LO_Element* eptr, int32 position);

/*
 * Implements the UI policy for a click on an element. Returns TRUE if the result is an insertion point.
 */

Bool lo_ProcessClick(MWContext *context, lo_TopState *top_state, lo_DocState *state, LO_HitResult* result, Bool requireCaret);
Bool lo_ProcessDoubleClick(MWContext *context, lo_TopState *top_state, lo_DocState *state, LO_HitResult* result);
...
void
LO_StartSelection(MWContext *context, int32 x, int32 y)
{
#if 0
	int32 doc_id;
	...
	position = 0;
	eptr = lo_XYToDocumentElement(context, state, x, y, FALSE, TRUE,
		&ret_x, &ret_y);
	if (eptr != NULL)
	{
...
#endif
    LO_Click(context, x, y, ! EDT_IS_EDITOR(context)); /* Only allow selections in editor */
}

/*
 * Used for right-clicks.
 */
void LO_SelectObject( MWContext *context, int32 x, int32 y) {
	int32 doc_id;
	lo_TopState *top_state;
	lo_DocState *state;
    LO_HitResult result;
    ...
    LO_Hit(context, x, y, FALSE, &result);

    if ( ! lo_ProcessAnchorClick(context, top_state, state, &result ) ) {
        lo_ProcessClick(context, top_state, state, &result, FALSE );
    }
}


void LO_StartSelectionFromElement( MWContext *context, LO_Element *eptr, int32 new_pos )
{
	int32 doc_id;
	lo_TopState *top_state;
	lo_DocState *state;
...
LO_Element*
lo_CellJumpingNext(MWContext *context, lo_DocState *state, LO_Element *eptr)
{
    /*
...
LO_Element*
lo_CellJumpingPrev(MWContext *context, lo_DocState *state, LO_Element *eptr)
{
    /*
     * If no previous element, see if we need to jump the cell wall.
     */
    LO_Element* prev = eptr->lo_any.prev;
    while ( eptr && ! prev  )
    {
        LO_Element* oldEptr = eptr;
        eptr = lo_JumpCellWall(context, state, eptr);
		...
		LO_Element **new_sel, int32 *new_pos, Bool forward)
{
	...
	if (forward != FALSE)
	{
		...
		{
			eptr = lo_CellJumpingNext(context, state, eptr);
            if (eptr == NULL)
			{
				eptr = sel;
			}
	...
	else
	{
		...
		if (eptr->lo_any.type != LO_TEXT)
		{
			eptr = lo_CellJumpingPrev(context, state, eptr);
    ...
    else
    {
        ...
        if ( forward && endPosition == 0 &&
            ( end->type == LO_LINEFEED ||
                ((prev = lo_CellJumpingPrev(context, state, end)) &&  prev->type == LO_LINEFEED ) ) )
        {
            lo_BumpEditablePositionForward(context, state, &end, &endPosition);
        }
    }
    LO_StartSelectionFromElement( context, begin, beginPosition);
    ...
    Bool extendFromStart)
{
    ...
    {
        normalizedStart = lo_CellJumpingNext(context, state, normalizedStart);
...
lo_ExtendToIncludeHardBreak(MWContext* context, lo_DocState *state, LO_Selection* selection)
{
    ...
    if ( EDT_IS_EDITOR(context) )
    {
        ...
        else
        {
            ...
            {
                LO_Element* eol = lo_CellJumpingNext(context, state, end->element);
    ...
    LO_Element** pElement, int32* pPosition)
{
    ...
    if ( lo_IsAtStartOfLine(context, state, eptr, position) )
    {
        ...
        while ( eptr && lo_CellJumpingPrev(context, state, eptr) ) {
...
lo_ExtendSelectionToPosition2(MWContext *context, lo_TopState* top_state, lo_DocState *state, LO_Element* eptr, int32 position)
{
...

    {
        ...
        if ( compare == 0 )
        {
            ...
            lo_SetInsertPoint(context, top_state, anchorElement, anchorPosition);
        }
        else
        {
	        LO_Element *changed_start, *changed_end;
		...
		int32 last_id;
			...
			if (eptr->lo_text.text != NULL)
			{
			    ...
			    if ((eptr == start)||(eptr == end))
			    {
			        ...
			        if (on != FALSE)
			        {
				    COPY_COLOR(eptr->lo_text.text_attr->fg,
					 save_fg);
				    COPY_COLOR(eptr->lo_text.text_attr->bg,
					 save_bg);
				    COPY_COLOR(save_bg,
					eptr->lo_text.text_attr->fg);
				    COPY_COLOR(save_fg,
					eptr->lo_text.text_attr->bg);
			        }
                    ...
                    /* XP_ASSERT(p2 < eptr->lo_text.text_len ); */

					charset = ((LO_TextStruct *) eptr)->text_attr->charset;
					if (INTL_CharSetType(charset) == SINGLEBYTE)
					{
			        	lo_DisplaySubtext(context, FE_VIEW,
						(LO_TextStruct *)eptr, p1, p2, !on);
					}
					else /* ugly processing for multibyte here	*/
					{
						char *string;
						int n;
						...
						if (p1 <= p2)
						{
			        			lo_DisplaySubtext(context, FE_VIEW, (LO_TextStruct *)eptr, p1, p2, !on);
						}
					}
...
lo_BumpToNextElement(MWContext *context, lo_DocState* state, LO_Element** eptr, int32* ePositionPtr, Bool forward)
{
    ...
    if ( forward )
    {
        element = lo_CellJumpingNext(context, state, element);
    }
    else
    {
        element = lo_CellJumpingPrev(context, state, element);
    }
...
*/
void LO_PositionCaret(MWContext *context, int32 x, int32 y)
{
    LO_Click(context, x, y, TRUE);
}

void LO_DoubleClick(MWContext *context, int32 x, int32 y)
{
	int32 doc_id;
	lo_TopState *top_state;
	lo_DocState *state;
    ...
    LO_Hit(context, x, y, FALSE, &result);
    lo_ProcessDoubleClick(context, top_state, state, &result);
}
...
void lo_HitLine2(MWContext *context, lo_DocState *state, LO_Element* element, int32 position, int32 x, LO_HitResult* result)
{
    LO_Element* begin;
    LO_Element* end;
    ...
    for(;;)
    {
       ...
       /* Search backwards to find the beginning of the line. */
        ...
        if ( begin->type == LO_TABLE )
        {
            /* If this is a table, give up. We don't understand tables, yet. */
            return;
        }
    ...
    LO_HitResult* result)
{
    ...
    /* Clip Y to the last line of the document */
    {
        ...
        int32 endY;
        if ( ! lo_FindDocumentEdge(context, state, &endOfDocument, TRUE, TRUE) )
        {
            return;
        }
        endY = endOfDocument.element->lo_any.y
            + endOfDocument.element->lo_any.y_offset
            + endOfDocument.element->lo_any.height;
        if ( y >= endY ){
            y = endY - 1;
        }
    }
    ...
    /* Clip Y to the first line of the document */
    {
        ...
        int32 startY;
        if ( ! lo_FindDocumentEdge(context, state, &startOfDocument, TRUE, FALSE) )
        {
            return;
        }
         ...
         */
        startY = startOfDocument.element->lo_any.y;
        if ( y < startY ){
            y = startY;
        }
    }
...
Bool LO_Click(MWContext *context, int32 x, int32 y, Bool requireCaret)
{
    ...
    LO_Hit(context, x, y, requireCaret, &result);
...
lo_IsEdgeOfDocument2(MWContext* context, lo_DocState *state, LO_Element* element, int32 position, Bool forward)
{
    ...
    if ( forward )
    {
        ...
        if ( element->type != LO_LINEFEED
            ...
            return FALSE;
        next = lo_CellJumpingNext(context, state, element);
        if ( next == NULL ||
            lo_CellJumpingNext(context, state, next) == NULL )
            return TRUE;
    }
    else
    {
...
Bool
lo_FindEdgeOfWord(MWContext* context, lo_DocState *state, LO_Position* where, LO_Position* wordEdge, Bool bSelect, Bool forward)
{
    /*
...
{
    lo_FindEdgeOfWord(context, state, where, &word->begin, TRUE, FALSE);
    LO_ASSERT_POSITION(context, &word->begin);
    lo_FindEdgeOfWord(context, state, where, &word->end, TRUE, TRUE);
            ...
            case LO_HIT_ELEMENT_REGION_MIDDLE:
                {
                    ...
                    return lo_ProcessClick( context, top_state, state, result, FALSE);
                }
                break;
            default:
                break;
...
lo_FindChunkEdge(MWContext* context, lo_DocState *state, LO_Position* where, LO_Position* wordEdge, intn chunkType, Bool select, Bool forward)
{
    ...
    *wordEdge = *where;
    ...
    case LO_NA_CHARACTER:
        ...
        break;
    case LO_NA_WORD:
        result = lo_FindEdgeOfWord(context, state, where, wordEdge, select, forward);
        break;
    case LO_NA_LINEEDGE:
...
LO_NavigateChunk( MWContext *context, intn chunkType, Bool bSelect, Bool bForward )
{
    ...
    if ( bResult ) {
        ...
        else
        {
            ...
            lo_SetInsertPoint(context, top_state, caret.element, caret.position);
        }
    }
	...
	*/
	if( bFound ){
		if( bSelect ){
			...
			LO_PositionCaret( pContext, x, y );
		}
	}
    else {
        /* No more elements. */
        if ( bSelect ) {
            /* In select mode, select to the edge of the document. */
            LO_NavigateChunk( pContext, LO_NA_DOCUMENT, bSelect, bForward);
        }
        else
        {
            /* In navigate mode, move to the corresponding position
             * of the last line
             */
            LO_Position edge;
            if ( lo_FindDocumentEdge(pContext, state, &edge, bSelect, bForward) ) {
                LO_PositionCaret( pContext, iDesiredX, edge.element->lo_any.y);
            }
        }
    }
#endif

								   ...
								   XP_Bool go_backwards)
{
	...
	while((next_ele = lo_return_next_form_element(context, current_element, go_backwards)))
	  {
...
lo_BeginForm(MWContext *context, lo_DocState *state, PA_Tag *tag)
{
	...
	state->top_state->current_form_num++;

	form->id = state->top_state->current_form_num;
	form->form_ele_cnt = 0;
	...
	form->next = state->top_state->form_list;
	state->top_state->form_list = form;
...
lo_EndForm(MWContext *context, lo_DocState *state)
{
	...
	form_id = state->top_state->current_form_num;
	form_list = state->top_state->form_list;
...
new_form_element(MWContext *context, lo_DocState *state)
{
	...
	form_element->form_id = state->top_state->current_form_num;
		...
		if (state->is_a_subdoc == SUBDOC_CELL)
		{
			    ...
			    (up_state->current_ele->lo_subdoc.bg_color != NULL))
			{
				lo_recolor_form_element_bg(state, form_element,
				    up_state->current_ele->lo_subdoc.bg_color);
			}
		}
	}
...
lo_next_element_data(lo_DocState *state)
{
	...
	PA_Tag *tag, int32 type)
{
	...
	form_element = new_form_element(context, state);
	if (form_element == NULL)
	{
		return(NULL);
	}

	form_element->element_data = lo_next_element_data(state);
...
lo_form_select(MWContext *context, lo_DocState *state, PA_Tag *tag)
{
	...
	if ((form_data->multiple != FALSE)||(form_data->size > 1))
	{
...
#ifdef MOCHA
	lo_CompileEventHandler(context, state, tag, form_element, PARAM_ONFOCUS);
	lo_CompileEventHandler(context, state, tag, form_element, PARAM_ONBLUR);
	lo_CompileEventHandler(context, state, tag, form_element, PARAM_ONCHANGE);
#endif
...
lo_EndSelectTag(MWContext *context, lo_DocState *state)
{
	...
	if (form_element != NULL)
	{
		 ...
		 */
		if ((form_element->element_data != NULL)&&
			(form_element->element_data->type ==
			FORM_TYPE_SELECT_ONE))
		{
			Bool sel_set;
			int32 i;
...
lo_BeginTextareaTag(MWContext *context, lo_DocState *state, PA_Tag *tag)
{
	...
	lo_CompileEventHandler(context, state, tag, form_element, PARAM_ONFOCUS);
	lo_CompileEventHandler(context, state, tag, form_element, PARAM_ONBLUR);
	lo_CompileEventHandler(context, state, tag, form_element, PARAM_ONSELECT);
	lo_CompileEventHandler(context, state, tag, form_element, PARAM_ONCHANGE);
	lo_CompileEventHandler(context, state, tag, form_element, PARAM_ONSCROLL);
#endif
...
lo_ProcessInputTag(MWContext *context, lo_DocState *state, PA_Tag *tag)
{
	...
	lo_CompileEventHandler(context, state, tag, form_element, PARAM_ONSELECT);
	lo_CompileEventHandler(context, state, tag, form_element, PARAM_ONCHANGE);
	lo_CompileEventHandler(context, state, tag, form_element, PARAM_ONCLICK);
#endif
...
lo_get_form_element_data(MWContext *context,
		...
		uint8 *type_array, int32 *array_cnt)
{
	LO_FormElementStruct *form_element;
	PA_Block name;
	PA_Block value;
		...
		lo_FormData *form_list, LO_FormElementStruct *submit_element)
{
	...
	for (i=0; i<form_list->form_ele_cnt; i++)
	{
		...
		wait = lo_get_form_element_data(context,
				...
				name_array, value_array, type_array,
	...
	doc_csid = context->doc_csid;
	if (doc_csid == CS_DEFAULT)
		doc_csid = INTL_DefaultDocCharSetID(context);
	doc_csid &= (~CS_AUTO);

	if (INTL_GetCharCodeConverter(context->win_csid, doc_csid, object)) {
...
#ifdef MOCHA
	if (!LM_SendOnReset(context, (LO_Element *)form_element))
	{
		return;
	}
#endif
...
lo_ProcessIsIndexTag(MWContext *context, lo_DocState *state, PA_Tag *tag)
{
	...
	origFE = INTL_InternalToFontEncoding(charset, (unsigned char *) str);
	fe = origFE;
	while (fe)
	{

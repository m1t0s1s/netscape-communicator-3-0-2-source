	...
	int32 *pos, Bool forward)
{
	...
	if (eptr->type == LO_TEXT)
	{
			...
			(position < (eptr->lo_text.text_len - 1)))
		{
			...
			position = INTL_NextCharIdxInText(context->win_csid, (unsigned char*)eptr->lo_text.text, position );
#else
			position++;
#endif
			*ele_loc = eptr;
...
#ifdef INTL_FIND
			/*	Add by ftang to provide international find */
			position = INTL_PrevCharIdxInText(context->win_csid, (unsigned char*)eptr->lo_text.text, position );
#else
			position--;
#endif
			*ele_loc = eptr;
		...
		*ele_loc = eptr;
		...
		else
		{
			...
			if(eptr->lo_text.text_len == 0)
				...
				position = INTL_PrevCharIdxInText(context->win_csid, (unsigned char*)eptr->lo_text.text, eptr->lo_text.text_len );
#else
			position = eptr->lo_text.text_len - 1;
			if (position < 0)
			{
...
#ifdef INTL_FIND
					if (use_case)
						have_start = INTL_MatchOneCaseChar(context->win_csid, (unsigned char*)cmp_text,tptr,&charlen);
					else
						have_start = INTL_MatchOneChar(context->win_csid, (unsigned char*)cmp_text,tptr,&charlen);
#else
					if (use_case == FALSE)
					{
						tchar = TOLOWER(*tptr);
...
#ifdef INTL_FIND
					if (use_case)
						not_equal = ! INTL_MatchOneCaseChar(context->win_csid,(unsigned char *) cmp_text+cnt,tptr,&charlen);
					else
						not_equal = ! INTL_MatchOneChar(context->win_csid,(unsigned char *) cmp_text+cnt,tptr,&charlen);
#else
					if (use_case == FALSE)
					{                          
					...
					LO_Element *eptr)
{
	 ...
	 */
	cell = lo_XYToDocumentElement(context, state, x, y, TRUE, FALSE,
					&ret_x, &ret_y);

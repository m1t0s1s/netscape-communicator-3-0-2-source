	...
	*rows = cnt;
	...
	for (i=0; i<cnt; i++)
	{
		...
		if (*end_ptr == '*')
		{
			...
			percent_list[i].value = (int32)XP_ATOI(tptr);
			if (percent_list[i].value < 1)
			{
				percent_list[i].value = 1;
			}
		}
		else if (*end_ptr == '%')
		{
			...
			percent_list[i].value = (int32)XP_ATOI(tptr);
			if (percent_list[i].value <= 0)
			{
				percent_list[i].value = 100 / cnt;
				if (percent_list[i].value == 0)
				{
					percent_list[i].value = 1;
				}
...
void
lo_FreeGridCellRec(MWContext *context, lo_GridCellRec *cell)
{
	if (cell == NULL)
	{
		return;
	...
	if (cell->hist_list != NULL)
	{
		...
		while (hist != NULL)
		{
			...
			{
				SHIST_FreeHistoryEntry(cell->context, hist);
			}
			/*
			 * If the cell has no context, but still has a history,
			 * pass the parent's context.  This is bogus but
			 * needed to keep the winfe from dumping because it
			 * blindly dereferences the context even though it
			 * doesn't use it.
			 */
			else
			{
				SHIST_FreeHistoryEntry(context, hist);
			}
			hist =(History_entry *)XP_ListRemoveTopObject(list_ptr);
		}
...
#else
	if (cell->history != NULL)
	{
		if (cell->context != NULL)
		{
			SHIST_FreeHistoryEntry(cell->context,
				(History_entry *)cell->history);
		}
		/*
		 * If the cell has no context, but still has a history,
		 * pass the parent's context.  This is bogus but needed to
		 * keep the winfe from dumping because it blindly
		 * dereferences the context even though it doesn't use it.
		 */
		else
		{
			SHIST_FreeHistoryEntry(context,
				(History_entry *)cell->history);
		}
	}
...
lo_adjust_percents(int32 count, lo_GridPercent *percents, int32 max)
{
	...
	for (i=0; i < count; i++)
	{
		if (percents[i].type == LO_PERCENT_FIXED)
		{
			 ...
			 */
			percents[i].value = 100 * percents[i].value / max;
		...
		{
			free_percent += percents[i].value;
		}
		else
		{
			total += percents[i].value;
		}
	}
	...
	lo_GridRec *grid, lo_GridEdge **edges)
{
	...
	while (cell_list != NULL)
	{
		...
		tmp_edge = NULL; /* get whiny gcc to shut up */
		...
		if (cell_list->subgrid != NULL)
		{
			...
			lo_FreeGridCellRec(context, dummy_cell);
		}
		/*
...
lo_set_all_edge_bounds(MWContext *context, lo_DocState *state, lo_GridRec *grid)
{
	...
	while (edge_list != NULL)
	{
		...
		lo_DisplayEdge(context, FE_VIEW, edge_list->fe_edge);
		edge_list = edge_list->next;
	}
}
...
lo_MakeGrid(MWContext *context, lo_DocState *state, lo_GridRec *grid)
{
	...
	if (state->top_state->old_grid != NULL)
	{
		...
		while ((cell_ptr != NULL)&&(old_cell_ptr != NULL))
		{
			 ...
			 */
			cell_ptr->history = old_cell_ptr->history;
			old_cell_ptr->history = NULL;
#ifdef NEW_FRAME_HIST
...
#ifdef NEW_FRAME_HIST
			/*
			 * There is a race condition such that the GetURL
			 * for the above new window may complete, and add
			 * its first history entry before the call to 
			 * FE_MakeGridWindow ever returns.  (This usually
			 * happens on file: urls.)  In this case, the call
			 * to LO_UpdateGridHistory() would have failed
			 * because the cell's context pointer wasn't set yet.
			 * We attempt to detect that case below, and correct it
			 * by calling into LO_UpdateGridHistory() again.
			 */
			if ((cell_ptr->context != NULL)&&
			    (cell_ptr->hist_indx == 0)&&
			    (SHIST_GetCurrent(&(cell_ptr->context->hist)) != NULL))
			{
				LO_UpdateGridHistory(cell_ptr->context);
			}

			/*
			 * If we have an old history list for this cell,
			 * and we have a context, restore that history
			 * into that context now.
			 */
			if ((cell_ptr->context != NULL)&&
				(cell_ptr->hist_list != NULL))
			{
				XP_ListDestroy(cell_ptr->context->hist.list_ptr);
				cell_ptr->context->hist.list_ptr =
					cell_ptr->hist_list;
			}
			/*
			 * Reset this cell to the overall grid's
			 * current history location, and load that
			 * doc into the cell.
			 */
			if (cell_ptr->history != NULL)
			{
				XP_InterruptContext(cell_ptr->context);

				cell_ptr->hist_indx = grid->current_hist;
				SHIST_SetCurrent(&(cell_ptr->context->hist),
				    SHIST_GetIndex(&(cell_ptr->context->hist),
					(History_entry *)cell_ptr->hist_array[cell_ptr->hist_indx - 1].hist));
	...
	int32 bound, int32 diff)
{
	if (edge != NULL)
	{
		...
		if ((edge->x + edge->width) == bound)
		{
			 ...
			 */
			lo_DisplayEdge(context, FE_VIEW, edge->fe_edge);
		}
	}
}
	...
	int32 *margin_width, int32 *margin_height)
{
	...
	/*
	 * If we cound the cell, set our margins.
	 */
	if (cell_ptr != NULL)
	{
...
LO_UpdateGridHistory(MWContext *context)
{
	...
	cell_ptr = lo_ContextToCell(context, &grid);
	if (cell_ptr == NULL)
	{
		return;
	}
	...
	cell_ptr->hist_array[cell_ptr->hist_indx].hist = (void *)hist;
	if (cell_ptr->hist_indx == 0)
	{
...
lo_cleanup_grid_history(MWContext *context, lo_GridRec *grid)
{
	...
	while (cell_ptr != NULL)
	{
		...
		if ((list_ptr != NULL)&&(cell_hist != NULL))
		{
			...
			SHIST_FreeHistoryEntry(context, old_entry);
		    }
		}

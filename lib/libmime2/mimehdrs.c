...
int 
MimeHeaders_map_headers (MimeHeaders *hdrs,
  ...
  for (i = 0; i < hdrs->heads_size; i++)
	{
	  ...
	  /* save the old colon value, and null-terminate the name... */
	  saved_colon = *colon;
	  *colon = 0;

	  /* save the old terminating newline, and null-terminate the value. */
	  saved_nl = *end;
	  *end = 0;

	  /* Run the callback with a pair of null-terminated strings. */
	  status = mapper (head, contents, closure);

	  /* Restore the characters we stomped. */
	  *colon = saved_colon;
	  *end = saved_nl;
...

#if defined(MIME_TO_HTML) && defined(XP_UNIX)

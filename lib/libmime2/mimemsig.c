...
MimeMultipartSigned_initialize (MimeObject *object)
{
  ...
  XP_ASSERT(!object->options || !object->options->write_leaf_p);
  ...
  if (!abort_p)
	{
	  /* Now that we've read both the signed object and the signature (and
		 ...
		 We can get here without having a crypto_closure in the case of
		 forwarding a signed message; in that case, MimeMessage hasn't
		 bothered to feed it's child (this MultipartSigned object) any
		 lines, but has instead written them out raw.  However, the
		 child object has still been created, and still gets eof, so
		 we get here without having seen any lines.

		 Arguably MimeMessage shouldn't be creating a child object at all
		 in this case, but I'm not totally sure what other implications
		 that might have.
	   */
#ifdef MIME_TO_HTML
	  XP_ASSERT (sig->crypto_closure ||
				 (obj->options && !obj->options->write_html_p));
#endif /* MIME_TO_HTML */

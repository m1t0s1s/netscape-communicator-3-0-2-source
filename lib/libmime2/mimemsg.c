...
#if defined(MIME_DRAFTS) && defined(MIME_SECURITY)
...
#endif /* MIME_DRAFTS && MIME_SECURITY */
...
# endif /* MIME_SECURITY */
	  ...
	  if (obj->options && obj->options->write_leaf_p)
		{
		  MimeMessage *msg = (MimeMessage *) obj;
		  XP_ASSERT(!msg->msg_leaf_closure);
		  msg->msg_leaf_closure =
			obj->options->leaf_begin (obj->content_type,
									  obj->headers,		/* leaf headers */
									  msg->hdrs,		/* container headers */
									  obj->options->leaf_closure);
		  if (!msg->msg_leaf_closure)
	  ...
	  if (obj->output_p &&
		  ...
		  !mime_crypto_object_p (msg->hdrs, FALSE))
		{
		  /* The body of this message is not an encrypted object, so we need
			 to turn off the decrypt_p flag (to prevent us from sucking the
...
MimeMessage_parse_eof (MimeObject *obj, XP_Bool abort_p)
{
  ...
  if (msg->msg_leaf_closure)
	{
	  status = obj->options->leaf_end (msg->msg_leaf_closure, abort_p);
	  msg->msg_leaf_closure = 0;
	  if (status < 0) return status;
	}
#endif /* MIME_LEAF_TEXT */
  ...
  if (msg->crypto_stamped_p)
...
#endif /* MIME_SECURITY */
	{
	  /* If we're not writing a crypto stamp, and this is the outermost
		 ...
		 closed off, in MimeEncrypted_emit_buffered_child() or

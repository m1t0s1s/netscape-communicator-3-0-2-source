...
#pragma message ("replace net_roundup with PR_roundup")
#define NET_ROUNDUP(x, y)   ((((x)+((y)-1))/(y))*(y))

	if(net_cache_file_block_size == 0)
		return(size);
	else  /* round sizes up to the next block size */
		return(NET_ROUNDUP(size, net_cache_file_block_size));
			   ...
			   store_type_enum 	 store_type)
{
	...
	if(!cache_database)
	  {
		 ...
		 */
		XP_FileRemove(obj->filename, xpCache);
	...
	if(store_type == NEW_CACHE_STORE)
	  {
        ...
        else if(XP_Stat(obj->filename, &stat_entry,  xpCache) != -1)
          {
		    if(stat_entry.st_size == 0)
		      {
	...
	if(store_type == NEW_CACHE_STORE)
		status = (*cache_database->put)(cache_database, 
										key, 
										data, 
										R_NOOVERWRITE);
	else /* overwrite by default */
		status = (*cache_database->put)(cache_database, key, data, 0);
    ...
    if(status > 0)
	  {
		...
		status = (*cache_database->get)(cache_database, key, &old_data, 0);
	...
	if (++cache_sync_count >= CACHE_SYNC_RATE) 
	{
		 ...
		 * consistancy
		 */
	    if(cache_database && -1 == (*cache_database->sync)(cache_database, 0))
	      {
	        TRACEMSG(("Error syncing cache database"));
 ...
 */
PRIVATE unsigned int net_CacheWriteReady (CacheDataObject * obj)
{
   if(obj->next_stream)
 ...
 */
PRIVATE int net_CacheWrite (CacheDataObject * obj, CONST char* buffer, int32 len)
{
    ...
    if(obj->fp)
	  {
		 ...
		 */
        if(XP_FileWrite((char *) buffer, len, obj->fp) < len)
		  {
...
/*  complete the stream
*/
PRIVATE void net_CacheComplete (CacheDataObject * obj)
{
 ...
 * 
 */
PRIVATE void net_CacheAbort (CacheDataObject * obj, int status)
{
		...
		if(status == MK_INTERRUPTED 
			&& obj->URL_s
			 && obj->URL_s->content_length
			  && obj->URL_s->server_can_do_byteranges
                    ...
                    MWContext  *window_id)
{
     ...
     */
	if (NET_URL_Type(URL_s->address) != WYSIWYG_TYPE_URL)
    ...
    if(URL_s->must_cache	/* overrides all the rest of these tests */
	   ||  (CLEAR_CACHE_BIT(format_out) == FO_INTERNAL_IMAGE 
			...
			|| !strcasecomp(URL_s->content_type, APPLICATION_HTTP_INDEX)
			    /* always cache small things, regardless of type */
			|| ( (URL_s->content_length>0) && (URL_s->content_length<0x10000) )
		   )
	   )
	  {
		...
		if (filename)  
		  {
			...
			if (cache_object->address)
			  {
				char *tail = XP_STRRCHR (cache_object->address, '/');
				char *suffix = (tail ? XP_STRRCHR (tail, '.') : 0);
				char *end = suffix + (suffix ? XP_STRLEN (suffix) : 0);
				char *junk=0;
				char old_char;
			...
			if(!last_modified_date)
			  {
				 ...
				 */
				if(number_in_remove_list < REMOVE_LIST_SIZE)
			...
			if(long_form)
			  {
				...
				TABLE_TOP("Uses relative path:");
				XP_SPRINTF(buffer, "%s", cache_obj->is_relative_path ? 
													"TRUE" : "FALSE");
				PUT_PART(buffer);
				TABLE_BOTTOM;

				TABLE_TOP("From Netsite Server:");
...
found:
	fromfp = XP_FileOpen(filename, xpCache, XP_FILE_READ_BIN);
	if (!fromfp)
		return NULL;

...
PRIVATE char *net_BeginColorHTMLTag (DataObject *obj)
{
	...
	if (obj->tag_type == P_SCRIPT)
	  {
		StrAllocCopy(new_markup, "</XMP><PRE WRAP>"); /* jwz: wrap */
...
PRIVATE int net_ColorHTMLWrite (DataObject *obj, CONST char *s, int32 l)
{
	int32 i;
 ...
 */
PRIVATE unsigned int net_ColorHTMLWriteReady (DataObject * obj)
{
	return((*obj->next_stream->is_write_ready)(obj->next_stream->data_object));
}


PRIVATE void net_ColorHTMLComplete (DataObject * obj)
{
	(*obj->next_stream->complete)(obj->next_stream->data_object);
}

PRIVATE void net_ColorHTMLAbort (DataObject * obj, int status)
{
                     ...
                     MWContext  *window_id)
{
	...
	StrAllocCopy(new_markup, "<TITLE>Source of: ");
	...
	if(!is_html_stream)
		...
		StrAllocCat(new_markup, "<PRE WRAP>"); /* jwz: wrap */

	FREE(old_url);

  	status = (*next_stream->put_block)(next_stream->data_object,
        									new_markup,
	...
	if(status < 0)
	  {
  		(*next_stream->abort)(next_stream->data_object, status);
		FREE(next_stream);
		return(NULL);
	  }

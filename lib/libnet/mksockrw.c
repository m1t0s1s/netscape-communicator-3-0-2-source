 ...
 */
#define TURN_SECURITY_ON
#endif

#ifdef XP_UNIX
/* #### WARNING, this crap is duplicated in mkconect.c
 */
# include <sys/ioctl.h>
/*
 ...
 *
 */
PUBLIC int NET_HTTPNetWrite (NETSOCK fildes, CONST void * buf, unsigned int nbyte)
{
    static int status;
	 ...
	 */
    if ((status < 0) 
		 && (status == ENOTCONN || status == ECONNRESET || status == EPIPE)) 
...
PUBLIC int32 NET_BlockingWrite (NETSOCK filedes, CONST void * buf, unsigned int nbyte)
{
	...
	while(nbyte > 0 && length_written > -1)
	  {
		length_written = NETWRITE(filedes, (const char*) buf+tot_len_written, nbyte);
 ...
 */
MODULE_PRIVATE int 
NET_DebugNetWrite (NETSOCK fildes, CONST void *buf, unsigned nbyte)
{
  ...
  return(SOCKET_WRITE(fildes, buf, nbyte));
}
...
NET_DebugNetRead (NETSOCK fildes, void * buf, unsigned nbyte)
{
  ...
  status = SOCKET_READ (fildes, buf, nbyte);

  if(MKLib_trace_flag && status > SOCKET_INVALID)
    {
#ifdef XP_UNIX
      write(2,"Rx: ", 4);
 ...
 */
MODULE_PRIVATE int 
NET_HTTPNetRead (NETSOCK fildes, void *buf, unsigned nbyte)
{
  static int status;  /* read return code */
...
MODULE_PRIVATE int
NET_BufferedReadLine   (NETSOCK   sock, 
             	        char   ** line, 
             	        char   ** buffer, 
             	        int32   * buffer_size, 
    ...
    if(!linefeed)
      {
        ...
        status = SOCKET_READ(sock, NET_Socket_Buffer, read_size);

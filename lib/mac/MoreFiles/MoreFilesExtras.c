								  ...
								  StringPtr copyName)
{
	...
	if ( error == paramErr || error == noErr )	/* paramErr is OK, it just means this volume doesn't support GetVolParms */
	{
		...
		else
		{
			...
			if ( error == noErr )
			{
				/* CHANGE: check if it's a folder */
				if (isDirectory)
				{
					OSErr	moveError = noErr;
					
					/* move it to the new location if necessary */
					if (srcDirID != realDstDirID)
						moveError = CatMove(vRefNum, srcDirID, srcName, dstDirID, dstpathName);
					
					/* rename it if names are different */
					if ( memcmp( srcName, copyName, srcName[0] ) != 0 )
					{
						error = HRename(vRefNum, (moveError == noErr) ? realDstDirID : srcDirID, srcName, copyName);
					
						/* if we had a problem moving before we'll try again now with new name */
						if ( moveError )
							moveError = CatMove(vRefNum, srcDirID, copyName, dstDirID, dstpathName);
					}

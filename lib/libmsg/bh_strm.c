...
*/
PRIVATE int net_BinHex_Encode_Write (void *closure, const char* s, int32 l)
{
	int  	status = 0;
	BinHexEncodeObject * obj = (BinHexEncodeObject*)closure;
	int32 	count;
	int32	size;
...
PRIVATE unsigned int net_BinHex_Encode_Ready (void *closure)
{
   return(MAX_WRITE_READY);  					/* always ready for writing 		*/ 
}


PRIVATE void net_BinHex_Encode_Complete (void *closure)
{
	int32	count;
	BinHexEncodeObject *obj = (BinHexEncodeObject *)closure;
	...
	**	push the close part.
	*/
	binhex_encode_next(&(obj->bh_encode_obj), 
							NULL,
							0,
							obj->buff, 
							obj->s_buff, 
...
PRIVATE void net_BinHex_Encode_Abort (void * closure, int status)
{
	BinHexEncodeObject * obj = (BinHexEncodeObject*)closure;
...
{
	int		status = NOERR;
	BinHexDecodeObject * obj = (BinHexDecodeObject*) closure;
	int32 	size;
 ...
 */
PRIVATE unsigned int net_BinHex_Decode_Ready (void * closure)
{
   return(MAX_WRITE_READY); 	 						/* always ready for writing */ 
}


PRIVATE void net_BinHex_Decode_Complete (void *closure)
{
	BinHexDecodeObject *obj = (BinHexDecodeObject *)closure;
...
PRIVATE void net_BinHex_Decode_Abort (void *closure, int status)
{
	BinHexDecodeObject *obj = (BinHexDecodeObject *)closure;

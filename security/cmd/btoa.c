...
static int BTOAFile(FILE *outFile, FILE *inFile)
{
    BTOAContext *cx;
    unsigned part;
    int nb;
	...
	rv = BTOA_Update(cx, obuf, &part, sizeof(obuf), ibuf, nb);
	if (rv) goto loser;
    ...
    rv = BTOA_End(cx, obuf, &part, sizeof(obuf));
    if (rv) goto loser;
    if (part) {
	nb = fwrite(obuf, 1, part, outFile);
	if (nb != part) {

...
#ifndef lint
static char RCSid[] = 
"$Id: rshrcmd.c,v 1.2 1996/04/24 00:12:43 dkarlton Exp $";
#endif
...
#ifdef USE_SSL
	if ( secargs && keyflag && !dongleflag) {
	    write(sp[0], password, strlen(password)+1);
	}
		...
		if (strcmp(*ahost, "localhost") == 0 &&
			...
			if ( keyflag && !keydir ) {
			    execlp(path_remsh, basename(path_remsh), 
				   "-S", secargs, "-k", nickname,
				   "-l", ruser, *ahost, cmd, (char *) NULL);
			} else if ( keydir && !keyflag ) {
			    execlp(path_remsh, basename(path_remsh), 
				   "-S", secargs, "-K", keydir,
				   *ahost, "-l", ruser, cmd, (char *) NULL);
			} else if ( keydir && keyflag ) {
			    execlp(path_remsh, basename(path_remsh),
				   "-S", secargs, "-K", keydir, "-k", nickname,
				   "-l", ruser, *ahost, cmd, (char *) NULL);
			} else {
			    execlp(path_remsh, basename(path_remsh),
				   "-S", secargs,

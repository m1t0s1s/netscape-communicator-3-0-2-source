 ...
 *
 * $Id: p7decode.c,v 1.11.2.15 1997/05/12 02:31:41 repka Exp $
 */

#include "p7local.h"

#include "cert.h"
#ifdef XP_MAC			/* XXX do not want to have to include */
#include "certdb.h"		/* certdb.h -- the trust stuff needed by */
#else				/* the add certificate code needs to get */
#include "../cert/certdb.h"	/* rewritten/abstracted and then this */
#endif				/* include should be removed! */
#include "crypto.h"
#include "key.h"
#include "secasn1.h"
#include "secitem.h"
...
extern int SEC_ERROR_DECRYPTION_DISALLOWED;
extern int SEC_ERROR_NO_MEMORY;
extern int SEC_ERROR_NOT_A_RECIPIENT;
extern int SEC_ERROR_PKCS7_KEYALG_MISMATCH;
extern int SEC_ERROR_PKCS7_BAD_SIGNATURE;
extern int SEC_ERROR_UNSUPPORTED_KEYALG;

/*
 * XXX See comment where this function is called.  It is temporary and
 * should be removed when the explicit call itself is removed.
 */
extern PRBool
			     ...
			     PRBool final)
{
     ...
     * Decrypt this chunk.
     *
     * #### If we get an error, we do not want to do the digest or callback,
	...
	if (p7dcx->cb != NULL) {
	    ...
	    oldlen = plain->len;
	    if (oldlen == 0) {
		buf = PORT_ArenaAlloc (p7dcx->cinfo->poolp, buflen);
	    } else {
		buf = PORT_ArenaGrow (p7dcx->cinfo->poolp, plain->data,
				 ...
				 SECAlgorithmID **digestalgs)
{
    ...
    p7dcx->worker.digcxs = PORT_ArenaAlloc (p7dcx->tmp_poolp,
					    digcnt * sizeof (void *));
    p7dcx->worker.digobjs = PORT_ArenaAlloc (p7dcx->tmp_poolp,
				  ...
				  SECItem ***digestsp)
{
     ...
     */
    digests = PORT_ArenaAlloc (poolp, (worker->digcnt + 1) * sizeof(SECItem *));
    digest = PORT_ArenaAlloc (poolp, worker->digcnt * sizeof(SECItem));
	...
	digest->data = PORT_ArenaAlloc (poolp, digobj->length);
	if (digest->data == NULL) {
	    p7dcx->error = PORT_GetError();
	    PORT_ArenaRelease (poolp, mark);
	    return SECFailure;
				 ...
				 PK11SymKey **copy_key_for_signature)
{
    ...
    PK11SymKey *bulkkey;
    SECOidTag keyalgtag, bulkalgtag;
    sec_PKCS7CipherObject *decryptobj;
    PK11SlotInfo *slot;
    int i;
    ...
    cert = NULL;	/* quiet gcc warning about used uninitialized */
    for (i = 0; (ri = recipientinfos[i]) != NULL; i++) {
	cert = PK11_FindCertByIssuerAndSN (&slot, ri->issuerAndSN,
					   p7dcx->pwfn_arg);
	if (cert != NULL) {
	    privkey = PK11_FindPrivateKeyFromCert (slot, cert, p7dcx->pwfn_arg);
	    if (privkey != NULL) {
	        PK11_FreeSlot (slot);
		break;
	    }
    ...
    if (keyalgtag != SECOID_GetAlgorithmTag (&(ri->keyEncAlg))) {
    ...
    /*
     * XXX This is BAD, because it assumes that the only user of inline
     * PKCS7 decryption is SMIME.  Which is true right now, but *not*
     * how this code should be structured.  The fix for this is to have
     * the caller of SEC_PKCS7DecoderStart() pass in the following
     * function (with the API possibly modified, as appropriate) as a
     * callback.  The callback should be stored in p7dcx for us to get
     * at it here, and would then work basically the same way (we call
     * it to ask the question of whether this decryption is allowed).
     * Callers of the Decoder who do not expect to do any decryption
     * (e.g. those who "know" what they have is a signature, etc.)
     * should pass in NULL for the callback, and if we find here that
     * the callback is NULL we just assume the decryption is not allowed.
     */
    if (SECMIME_DecryptionAllowed (&(enccinfo->contentEncAlg),
				   bulkkey) == PR_FALSE) {
...
sec_pkcs7_decoder_notify (void *arg, PRBool before, void *dest, int depth)
{
	 ...
	 * an input that can be BER encoded.  Whoever thought that
	 * was an acceptable constraint for PKCS7 was batty, but
	 * there you have it.
	 *
	...
	/*
	 * We can only treat this like plain data because we do not have
	 * the key (it is supposed to be supplied by other means).
	 */
	/* fallthru */
		      ...
		      SECKEYGetPasswordKey pwfn, void *pwfn_arg)
{
    ...
    cinfo = PORT_ArenaZAlloc (poolp, sizeof(*cinfo));
    if (cinfo == NULL) {
    ...
    p7dcx = PORT_ZAlloc (sizeof(SEC_PKCS7DecoderContext));
    if (p7dcx == NULL) {
	PORT_FreeArena (poolp, PR_FALSE);
	return NULL;
    }
		    ...
		    SECKEYGetPasswordKey pwfn, void *pwfn_arg)
{
    ...
    p7dcx = SEC_PKCS7DecoderStart(cb, cb_arg, pwfn, pwfn_arg);
 ...
 * but that has a lousy error string associated with it right now, which
 * is too hard to change at this date, but it should be fixed eventually.
 *
			   ...
			   PRBool keepcerts)
{
     ...
     * but what the hell are the semantics here (like, return value)?
     */
    ...
    publickey = CERT_ExtractPublicKey (&cert->subjectPublicKeyInfo);
    if (publickey == NULL)
	goto done;
    ...
    /*
     * Find or confirm digest algorithm.
     */
    ...
    /*
     * XXX The only signature algorithm we (currently) support is RSA,
     * and this algorithm is what is assumed by VFY_Verify{Data,Digest}.
     */
	...
	goodsig = (VFY_VerifyData (encoded_attrs.data, encoded_attrs.len,
	...
	goodsig = (VFY_VerifyDigest (digest, publickey, sig, cinfo->pwfn_arg)
		   == SECSuccess);
...
sec_pkcs7_get_signer_cert_info(SEC_PKCS7ContentInfo *cinfo, int selector)
{
	...
	container = CERT_GetCertEmailAddress (&signercert->subject);
	break;
      default:
	PORT_Assert (0);
	container = NULL;

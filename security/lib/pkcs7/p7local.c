 ...
 *
 * $Id: p7local.c,v 1.11.2.2 1997/06/17 16:40:36 repka Exp $
 */
...
struct sec_pkcs7_cipher_object {
    ...
    int block_size;
    int pending_count;
    unsigned char pending_buf[MAX_BLOCK_SIZE];
};
 ...
 * int general...  I tried to keep the pkcs7-specific stuff (mostly
...
sec_PKCS7CreateDecryptObject (PK11SymKey *key, SECAlgorithmID *algid)
{
    ...
    result = PORT_ZAlloc (sizeof(struct sec_pkcs7_cipher_object));
    if (result == NULL)
	return NULL;
    ...
    result->block_size = PK11_GetBlockSize(mechanism,param);
			...
			PRBool final)
{
    ...
    /*
     * First, count the number of complete blocks of data we have.
     */
    blocks = (obj->pending_count + input_len) / block_size;

    /*
     * On the final call, add another block, which will contain any
		  ...
		  PRBool final)
{
	...
	PORT_Assert (padlen > 0 && padlen <= bsize);
	if (padlen == 0 || padlen > bsize) {
	    PORT_SetError (SEC_ERROR_BAD_DATA);
	    return SECFailure;
	}
	output_len -= padlen;
...
sec_PKCS7Encrypt (sec_PKCS7CipherObject *obj, unsigned char *output,
		  ...
		  PRBool final)
{
    int blocks, bsize, padlen, pcount;
    unsigned int max_needed, ifraglen, ofraglen, output_len;
    unsigned char *pbuf;
    SECStatus rv;
...
sec_attr_choose_attr_value_template(void *src_or_dest, PRBool encoding)
{
	...
	switch (oiddata->offset) {
	  ...
	  case SEC_OID_PKCS9_UNSTRUCTURED_NAME:
	    encoded = PR_FALSE;
	    template = SEC_IA5StringTemplate;
	    break;
	  case SEC_OID_PKCS9_CONTENT_TYPE:
	    encoded = PR_FALSE;
	    template = SEC_ObjectIDTemplate;
	    break;
	  case SEC_OID_PKCS9_MESSAGE_DIGEST:
	    encoded = PR_FALSE;
	    template = SEC_OctetStringTemplate;
	    break;
	  case SEC_OID_PKCS9_SIGNING_TIME:
	    encoded = PR_FALSE;
	    template = SEC_UTCTimeTemplate;
	    break;
...
sec_PKCS7ReorderAttributes (SEC_PKCS7Attribute **attrs)
{
     ...
     * a success because it also already has a find order.
     */
     ...
     */
    enc_attrs = PORT_ArenaZAlloc (poolp, num_attrs * sizeof(SECItem *));
    new_attrs = PORT_ArenaZAlloc (poolp,
...
sec_pkcs7_choose_content_template(void *src_or_dest, PRBool encoding)
{
	...
	template = SEC_PointerToOctetStringTemplate;
	break;
      case SEC_OID_PKCS7_SIGNED_DATA:
	template = SEC_PointerToPKCS7SignedDataTemplate;
	break;
      case SEC_OID_PKCS7_ENVELOPED_DATA:
	template = SEC_PointerToPKCS7EnvelopedDataTemplate;
	break;
      case SEC_OID_PKCS7_SIGNED_ENVELOPED_DATA:
	template = SEC_PointerToPKCS7SignedAndEnvelopedDataTemplate;
	break;
      case SEC_OID_PKCS7_DIGESTED_DATA:
	template = SEC_PointerToPKCS7DigestedDataTemplate;
	break;
      case SEC_OID_PKCS7_ENCRYPTED_DATA:
	template = SEC_PointerToPKCS7EncryptedDataTemplate;
	break;
    }
    return template;
}

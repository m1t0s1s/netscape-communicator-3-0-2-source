 ...
 *
 * $Id: p7create.c,v 1.10.2.7 1997/04/26 00:01:23 jwz Exp $
 */
			     ...
			     SECOidTag kind, PRBool detached)
{
	...
	cinfo->content.encryptedData = thing;
	...
	cinfo->content.envelopedData = thing;
		      ...
		      SECItem *digestdata)
{
    ...
    signerinfo = PORT_ArenaZAlloc (cinfo->poolp, sizeof(SEC_PKCS7SignerInfo));
    if (signerinfo == NULL) {
    ...
    signerinfos = PORT_ArenaAlloc (cinfo->poolp,
    ...
    digestalg = PORT_ArenaAlloc (cinfo->poolp, sizeof(SECAlgorithmID));
    ...
    if (digestdata != NULL) {
	digest = PORT_ArenaAlloc (cinfo->poolp, sizeof(SECItem));
	digests = PORT_ArenaAlloc (cinfo->poolp, 2 * sizeof(SECItem *));
			    ...
			    SECItem *value, PRBool encoded)
{
    ...
    attr = PORT_ArenaAlloc (poolp, sizeof(SEC_PKCS7Attribute));
    if (attr == NULL)
	goto loser;
    ...
    values = PORT_ArenaAlloc (poolp, 2 * sizeof(SECItem *));
    if (values == NULL)
	goto loser;
	...
	copy = PORT_ArenaAlloc (poolp, sizeof(SECItem));
	if (copy == NULL)
	    goto loser;
			 ...
			 SEC_PKCS7Attribute *attr)
{
	...
	for (count = 0; attrs[count] != NULL; count++)
	    ;
	attrs = PORT_ArenaGrow (cinfo->poolp, attrs,
	...
	*attrsp = attrs;
     ...
     */
    attrs = PORT_ArenaAlloc (cinfo->poolp, 4 * sizeof(SEC_PKCS7Attribute *));
    if (attrs == NULL)
	return SECFailure;
			   ...
			   CERTCertDBHandle *certdb)
{
	...
	if (signerinfo->cert != NULL)
	    signerinfo->certList = CERT_CertChainFromCert (certdb,
							   signerinfo->cert);
    }
			  ...
			  CERTCertDBHandle *certdb)
{
    ...
    certlist = CERT_CertChainFromCert (certdb, cert);
    if (certlist == NULL)
	return SECFailure;
			   ...
			   CERTCertificate *cert)
{
    ...
    if (certs == NULL) {
	count = 0;
	certs = PORT_ArenaAlloc (cinfo->poolp, 2 * sizeof(CERTCertificate *));
    } else {
	for (count = 0; certs[count] != NULL; count++)
	    ;
	PORT_Assert (count);	/* should be at least one already */
	certs = PORT_ArenaGrow (cinfo->poolp, certs,
			 ...
			 CERTCertDBHandle *certdb)
{
    ...
    recipientinfo = PORT_ArenaZAlloc (cinfo->poolp,
	...
	count = 0;
	recipientinfos = PORT_ArenaAlloc (cinfo->poolp,
					  2 * sizeof(SEC_PKCS7RecipientInfo *));
    } else {
	for (count = 0; recipientinfos[count] != NULL; count++)
	    ;
	PORT_Assert (count);	/* should be at least one already */
	recipientinfos = PORT_ArenaGrow (cinfo->poolp, recipientinfos,
 ...
 *
 * XXX I'm not sure that this is the way we want to represent the
 * bulk encryption and key size to use; but I can't think of anything
 * better at the moment.  We need to figure out how our caller
 * (e.g. libmsg) is going to know and specify this; we may want a
 * more abstract or indirect way.  But right now it looks like the
 * caller is going to have to know, based on looking over multiple
 * recipients, what they have in common.  So it can then just tell
 * us what was decided.  I don't want this interface to necessarily
 * assume S/MIME or user profiles.  But we could also define an
 * intermediate interface that did.  Help.
 *
 * The return value can be passed to functions which add things to
 ...
 */
SEC_PKCS7ContentInfo *
SEC_PKCS7CreateEncryptedData (SECOidTag algorithm,
			      SECKEYGetPasswordKey pwfn, void *pwfn_arg)
{
    SEC_PKCS7ContentInfo *cinfo;

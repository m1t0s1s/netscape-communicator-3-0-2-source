 ...
 *
 * $Id: secmime.c,v 1.1.2.7 1997/05/03 00:17:54 repka Exp $
 */

#include "secmime.h"
#include "secoid.h"
#include "pk11func.h"
#include "sslproto.h"	/* for CIPHER_FAMILY symbols */
#include "secasn1.h"
#include "secitem.h"
#include "cert.h"
#include "key.h"
...
SECMIME_EnableCipher(long which, int on)
{
	 ...
	 */
	smime_newprefs = PORT_ZAlloc (smime_symmetric_count
 ...
 * algorithm), find the appropriate policy algorithm specificiation
...
smime_policy_algorithm (SECAlgorithmID *algid, PK11SymKey *key)
{
      ...
      case SEC_OID_RC2_CBC:
	{
	    ...
	    keylen_bits = PK11_GetKeyStrength (key);
	    switch (keylen_bits) {
	      case 40:
...
smime_choose_cipher (CERTCertificate *scert, CERTCertificate **rcerts)
{
    ...
    cipher_abilities = PORT_ArenaZAlloc (poolp,
					 smime_symmetric_count * sizeof(int));
    if (cipher_abilities == NULL)
	goto done;

    cipher_votes = PORT_ArenaZAlloc (poolp,
				     smime_symmetric_count * sizeof(int));
	...
	if (profile != NULL && profile->data != NULL && profile->len > 0) {
	     ...
	     */
	    key = CERT_ExtractPublicKey (&rcerts[rcount]->subjectPublicKeyInfo);
	    if (key != NULL) {
 ...
 * Eventually, with more parameters needs to be specified, just
...
smime_init_caps (void)
{
    ...
    PORT_Assert (smime_encoded_caps == NULL || smime_prefs_changed);
	...
	smime_capabilities = PORT_ZAlloc ((smime_symmetric_count + 1)
	...
	if (cap == NULL) {
	    cap = PORT_ZAlloc (sizeof(smime_capability));
	    if (cap == NULL)
		break;
...
static SECStatus
smime_add_profile (SEC_PKCS7ContentInfo *cinfo)
{

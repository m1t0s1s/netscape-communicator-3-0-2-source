 ...
 *
 * $Id: p7encode.c,v 1.7.2.4 1997/06/17 16:42:02 repka Exp $
 */
...
#include "crypto.h"
#include "key.h"
#include "rsa.h"	/* XXX eventually this should be hidden from me? */
#include "secasn1.h"
#include "secoid.h"
#include "secrng.h"
#include "secitem.h"
						 ...
						 PK11SymKey *orig_bulkkey)
{
	...
	bulkkey = PK11_KeyGen(slot,type,enccinfo->keysize/8,cinfo->pwfn_arg);
	PK11_FreeSlot(slot);
	if (bulkkey == NULL) {
	    return NULL;
	}
	 ...
	 * of the following hardwired RSA stuff has *got* to go, or
	 * other kinds of keys/certs (like Fortezza) will not work.
	 */

	algtag=SECOID_GetAlgorithmTag(&(cert->subjectPublicKeyInfo.algorithm));

	rv = SECOID_SetAlgorithmID(cinfo->poolp, &ri->keyEncAlg, algtag, NULL);
	if (rv != SECSuccess)
	    goto loser;
	...
	case SEC_OID_PKCS1_RSA_ENCRYPTION:
	    ...
	    data_len = SECKEY_PublicKeyStrength(publickey);
						/*Only works for RSA*/
	    ri->encKey.data = PORT_ArenaAlloc(cinfo->poolp ,data_len);
	    ri->encKey.len = data_len;
	    if (ri->encKey.data == NULL) goto loser;

	    rv = PK11_PubWrapSymKey(PK11_AlgtagToMechanism(algtag),publickey,
				bulkkey,&ri->encKey);
				  ...
				  PK11SymKey *bulkkey)
{
    ...
    p7ecx = PORT_ZAlloc (sizeof(SEC_PKCS7EncoderContext));
    if (p7ecx == NULL)
	return NULL;
			     ...
			     PRBool final)
{
	...
	if (dest != NULL)
	    buf = PORT_ArenaAlloc (p7ecx->cinfo->poolp, buflen);
	else
	...
	digdata = PORT_ArenaAlloc (p7ecx->cinfo->poolp,
	...
	if (digestsp != NULL) {
	    ...
	    digest = PORT_ArenaAlloc (p7ecx->cinfo->poolp, sizeof(SECItem));
	    digests = PORT_ArenaAlloc (p7ecx->cinfo->poolp,
				       2 * sizeof(SECItem *));
...
{
    /* #### Error stuff needs help.  Return what?  Do "Finish" on failure? */
				 ...
				 SECKEYGetPasswordKey pwfn, void *pwfnarg)
{
     ...
     */
    rawcerts = PORT_ArenaAlloc (poolp, (certcount + 1) * sizeof(SECItem *));
    if (rawcerts == NULL)
	return SECFailure;

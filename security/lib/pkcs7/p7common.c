 ...
 *
 * $Id: p7common.c,v 1.6.2.8 1997/05/12 02:28:16 repka Exp $
 */
...
/*
 * XXX LISA still needs to fix the placement and formatting of the
			 ...
			 void *wincx)
{
    ...
    c_param.data = NULL;
    dest->data = PORT_ArenaZAlloc(poolp, (src->len + 64));
    dest->len = (src->len + 64);
    if(dest->data == NULL) {
	rv = SECFailure;
	goto loser;
    ...
    eKey = PK11_PBEKeyGen(slot, algid, key, wincx);
    if(eKey == NULL) {
	rv = SECFailure;
	goto loser;
    }
    ...
    rv = PK11_CipherOp(cx, dest->data, (int *)(&dest->len), 
			 ...
			 void *wincx)
{
    ...
    pbeMech.pParameter = result->data;
    pbeMech.ulParameterLen = result->len;
    if(PK11_MapPBEMechanismToCryptoMechanism(&pbeMech, &cryptoMech, key) != CKR_OK) {
	rv = SECFailure;
	goto loser;
    }
    c_param.data = (unsigned char *)cryptoMech.pParameter;

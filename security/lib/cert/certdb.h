...
#define CERT_DB_CONTENT_VERSION		3
...
typedef enum {
    ...
    certDBEntryTypeKeyRevocation = 5,	/* FORTEZZA */
...
/*
** Traverse the entire cert name database, and pass the names off to a
** user supplied function.
**	"namefunc" is the user function to call for each name
**	"udata" is the user's data, which is passed through to "namefunc"
*/
SECStatus
SEC_TraverseNames(CERTCertDBHandle *handle,
		  SECStatus (* namefunc)(SECItem *n, DBT *k, void *pdata),
		  void *udata );

...
extern int SEC_ERROR_SAFE_NOT_CREATED;
extern int SEC_ERROR_BAGGAGE_NOT_CREATED;
			    ...
			    void *wincx)
{
	...
	if((pk->espvkCipherText.pkcs8KeyShroud != NULL)  && (rv == SECSuccess)) {
	    ...
	    if(rv == SECSuccess) {
		SECItem *uniNickName = NULL;
		rv = SECITEM_CopyItem(poolp, &pk->espvkData.nickname, 
					nickname);
		uniNickName = sec_pkcs12_convert_item_to_unicode(nickname,
							swapUnicodeBytes);
		if(uniNickName != NULL) {
		    rv = SECITEM_CopyItem(poolp, &pk->espvkData.uniNickName,
					  uniNickName);
		    SECITEM_ZfreeItem(uniNickName, PR_TRUE);
		} else {
		    rv = SECFailure;
		}
	    }
 ...
 * transportMode -- an oid on how these things are transported
			 ...
			 void *wincx)
{
	...
	/* set up transport mode */
	oiddata = SECOID_FindOIDByTag(transportMode);
	if(oiddata != NULL) {
	    rv = SECITEM_CopyItem(asafe->poolp, &asafe->transportMode,
				 &oiddata->oid);
	} 

	if((rv == SECFailure) || (oiddata == NULL)) {
	    rv = SECFailure;
	    PORT_SetError(SEC_ERROR_NO_MEMORY);
	    goto loser;
	}
	...
	if(baggage != NULL) {
/*	    rv = sec_pkcs12_prepare_for_der_code_baggage(baggage, PR_TRUE);
	    if(rv != SECSuccess) {
		goto loser;
	    }
*/
		 ...
		 void *wincx)
{
	...
	cinfo = sec_pkcs12_get_auth_safe(safe, baggage, transportMode,
					 safe_alg, pwitem, wincx);

	sec_pkcs12_destroy_cert_content_infos(safe, baggage);

	/* get the pfx and mac it */
	if(cinfo != NULL) {
	    pfx = sec_pkcs12_get_pfx(cinfo, PR_TRUE, pwitem);
	    if(pfx != NULL) {

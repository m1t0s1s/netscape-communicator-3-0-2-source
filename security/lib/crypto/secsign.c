 ...
 *
 * $Id: secsign.c,v 1.35.2.6 1997/04/17 10:38:07 jwz Exp $
 */
...
SGN_NewContext(SECOidTag alg, SECKEYPrivateKey *key)
{
	...
	keyType = fortezzaKey;
	break;
      /* we don't implement MD4 hashes, do you think we're crazy... we
       * *CERTAINLY* don't want to sign one! */
...
SGN_End(SGNContext *cx, SECItem *result)
{
	...
	di = SGN_CreateDigestInfo(cx->alg, digest, part1);
	if (!di) {
	    rv = SECFailure;
	    goto loser;
	}
	...
	unsigned char *buf, int len, SECKEYPrivateKey *pk, SECOidTag algID)
{
	...
	rv = DSAU_DerSignData(&it, buf, len, pk, algID);
	if (rv) goto loser;
    } else {

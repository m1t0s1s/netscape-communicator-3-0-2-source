 ...
 *
 * $Id: keydb.c,v 1.48.2.7 1997/05/12 02:04:23 repka Exp $
 */
...
extern int SEC_ERROR_BAD_PADDING;
extern int SEC_ERROR_BAD_DER;
extern int SEC_ERROR_BAD_PASSWORD;
extern int SEC_ERROR_INVALID_ALGORITHM;
extern int SEC_ERROR_BAD_DATABASE;
...
get_dbkey(SECKEYKeyDBHandle *handle, DBT *index)
{
    ...
    rv = (* handle->db->get)(handle->db, index, &entry, 0);
    if ( rv ) {
 ...
 * XXX - should create a new database rather than doing it
...
seckey_create_rc4_salt(void)
{
	...
	ctxt = RNG_CreateContext();
	if(ctxt != NULL)
	{
	    RNG_Init(ctxt);
	    RNG_Reseed(ctxt);
	    RNG_GenerateRandomBytes(ctxt, salt->data, salt->len);
	    RNG_DestroyContext(ctxt, PR_TRUE);
	    rv = SECSuccess;
	}
    }
	...
	SECOidTag algorithm)
{
    ...
    PRArenaPool *temparena = NULL, *permarena = NULL;
    SECItem *key = NULL, *salt = NULL, *der_item = NULL;
    SECItem *dummy = NULL, *dest = NULL, params;
...
/*
 * Store a key in the database, indexed by its public key modulus.
 * Note that the nickname is optional.  It is only used by keyutil.
 */
SECStatus
...
/*
 * used by pkcs11 to import keys into it's object format (sigh... really
 *  need a better way to tie in...
 */
SECKEYLowPrivateKey *
...
static SECStatus
encodePWCheckEntry(PRArenaPool *arena, SECItem *entry, SECOidTag alg,
		   SECItem *encCheck)
{
    SECOidData *oidData;
    SECStatus rv;
			   ...
			   SECItem *pwitem, SECOidTag algorithm)
{
    ...
    SECItem *dest = NULL, test_key, *entry;
...
SECKEY_UpdateKeyDBPass2(SECKEYKeyDBHandle *handle, SECItem *pwitem)
{
    ...
    handle->checked = PR_TRUE;
    rv = SECKEY_ChangeKeyDBPassword(handle, pwitem, pwitem);
    handle->checked = PR_FALSE;
...
SECKEY_UpdateKeyDBPass1(SECKEYKeyDBHandle *handle)
{
    ...
    rv = (* handle->updatedb->get)(handle->updatedb, &checkKey,
				   &checkData, 0 );
    if (rv) {

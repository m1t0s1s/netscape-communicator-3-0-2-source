...
#define ALPHASHIFT	24L
#define REDSHIFT	16L
#define GREENSHIFT	8L
#define BLUESHIFT	0L
...
#define InitDstBuf(ird, x1, y1, x2, y2) \
      ...
      (dstAdjust = ird->GetBufScan() - (x2 - x1) * ird->GetDepth(colormodel) / 8L) >= 0)
/*
#define SetDstBufLoc(ird, x, y) \
    (dstP.vp = ird->GetDstBuf(),				\
    (dstP.bp += (((ird->GetHeight() - (y+1))*ird->GetBufScan())  + (x) * ird->GetDepth(colormodel) / 8)))
     //(dstP.bp += ((y) * ird->GetBufScan() + (x) * ird->GetDepth(colormodel) / 8)))
*/
#define StorePixel(ird, pixel, r, g, b) \
    ((ird->GetDepth(colormodel) == 24) \
    ? (*dstP.bp++ = b,	 	\
       *dstP.bp++ = g, 		\
       *dstP.bp++ = r)				\
    : (*dstP.bp++ = pixel))
...
#define SetMaskLoc(ird, x, y) \
    ((mask || (mask = ird->GetMaskBuf(FALSE)))				\
     && (maskp = (mask + ((ird->GetHeight() - (y+1))) * (((ird->GetWidth()+31)&(~31L)) >> 3L)	\
		  + ((x) >> 3L)),					\
	 maskbits = *maskp,						\
	 maskcurbit = (0x80L >> ((x) & 7L)),				\
	 maskadjust = ((((ird->GetWidth()+31)&(~31L)) >> 3L)		\
		       - ((dstX2 >> 3L) - (dstX1 >> 3L)))))



#define StartMaskLine() \
    (mask && (maskbits = *maskp, maskcurbit = (0x80L >> ((dstX1) & 7L))))
...
extern int GenericImageConvert(struct Hjava_awt_image_ColorModel *colormodel,
			       int32_t bgcolor, int32_t srcX, int32_t srcY, int32_t srcW, int32_t srcH,
			       void *srcpix, int32_t srcOff, int32_t srcBPP, int32_t srcScan,

...
class ThreadGroup {
	...
	if (destroyed) {
        ...
        ThreadGroup group = enumeratableThreadGroup();
        if (null == group) {
            return 0;
        }
        return group.activeCount0();
    }
    ...
    /**
     * Copies, into the specified array, references to every active Thread in this Thread group
     * that we allow the current thread to examine.
     * You can use the activeCount() method to get an estimate of how big
     * the array should be.
    ...
    public int enumerate(Thread list[], boolean recurse) {
        ThreadGroup legal_group = enumeratableThreadGroup();
        if (null == legal_group || (!recurse && this != legal_group))
            return 0;
        return legal_group.enumerate(list, 0, recurse);
    }
	...
	if (destroyed) {
        ...
        int n = 0;
        // add to list if "this" is a distant parent of legal_group
        ThreadGroup parents = legal_group;
        while (parents != this) {
            n++;
    ...
    /**
     * Copies, into the specified array, references to every active Thread group in this Thread 
     * group that we allow the current thread to examine.  The allowable list
     * includes both siblings and their descendents, and any parental ancestors that lead directly
     * from the current thread group up to "this" thread group.
     * You can use the activeGroupCount() method to get an estimate of how big
     * the array should be.
    ...
    public synchronized int enumerate(ThreadGroup list[], boolean recurse) {
        ...
        if (null == legal_group)
            return 0;
        int n = 0;
        // accumulate any (and all) of the ancestors of legal_group
        n = enumerateChildren(legal_group, list, recurse);
        if (!recurse && n > 0)
           return n;
        return legal_group.enumerate(list, n, recurse);
    }

    /**
     * Enumerate ancestral line of children down to but not including the argument, as 
     * though we did a traversal starting at "this" and working towards "child".
     * @parm child The last thread group we'll add to the list
     * @parm list  The accumulated ThreadGroups
     * @parm recurse Indicates that we should only consider the direct child of "this"
     * @return The number of entries placed into list.
     */
    private int enumerateChildren(ThreadGroup child, ThreadGroup list[], boolean recurse) {
        if (this == child) 
            return 0;
        ThreadGroup parent = child.parent;
        int n = enumerateChildren(parent, list, recurse);
    ...
    /**
     * Identify the largest thread group that can be scanned without violating
     * isolation of thread groups.  This means that we can scan either
     * our siblings, or decendants of our siblings.
     * @return A legally scannable threadgroup (or null).
     */
    private ThreadGroup enumeratableThreadGroup() {
	    ...
	    return this;                     // scan the (potentially) tiny selected portion

        if (this.parentOf(direct_parent))    
            return direct_parent;            // only list our siblings

        return null;                         // unrelated thread groups can't examine each other
    }
}

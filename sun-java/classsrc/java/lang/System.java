...
class System {
     ...
     * data.
     */
    final public static InputStream in = InputStreamInitializer(FileDescriptor.in);

    /**
     * Standard output stream. This stream is used for printing messages.
     */
    final public static PrintStream out = OutputStreamInitializer(FileDescriptor.out);
     ...
     */
    final public static PrintStream err = OutputStreamInitializer(FileDescriptor.err);
    ...
    /* The following bypasses the "final" restriction on its arguments.  It is called 
        from Console.java's native method (which is the real reason for its existance) */

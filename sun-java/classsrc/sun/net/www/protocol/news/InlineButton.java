/*
 * @(#)InlineButton.java	1.7 95/08/22 James Gosling, Jonathan Payne
 * 
...
class InlineButton extends Applet {
	...
	int minheight = fm.height + 4;
	try {
	    minheight = Integer.parseInt(getAttribute("height"));
	} catch (Exception e) {
	}
	int minwidth = labelWidth + 4;
	minwidth = (minwidth + 15) & ~15;
	try {
	    minwidth = Integer.parseInt(getAttribute("width"));
	} catch (Exception e) {
	}
	resize(minwidth, minheight);
	strX = (width - fm.stringWidth(label)) / 2;
	strY = (height - fm.height) / 2 + fm.ascent;
    }

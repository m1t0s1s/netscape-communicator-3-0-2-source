/*
 * @(#)newsFetcher.java	1.16 95/08/22 James Gosling, Jonathan Payne
 * 
...
class newsFetcher extends Thread {
	...
	try {
		...
		String serverEnvName = System.getProperty("nntp.server");
	...
	ps.print("<app class=sun.net.www.protocol.news.InlineButton label=\""
	...
	try {
		...
		/* AVH: I have no idea how to fix this
		try {
		...
		*/
	    ...
	    if (Newsgroups == null) {
		SecurityManager.setScopePermission();
		newsrcName = System.getProperty("user.home") + File.separator + ".newsrc";
		Newsgroups = Newsgroup.readNewsrcFile(newsrcName, news);
		if (Newsgroups == null) {
		    newsrcName = System.getProperty("user.home") + File.separator + ".hotjava"
		...
		if (Newsgroups == null || Newsgroups.length == 0) {
			...
			if (p.subscribed) {
			    ...
			    if (unread > 0)
				ps.print("<br><app class=sun.net.www.protocol.news.TotalUnReadIndicator group=" +
					 p.group.name +
					 classLoaderSrc +
	...
	ps.print("<dl compact>\n");
	while (n != null) {
	    ps.print("<dt><app class=sun.net.www.protocol.news.ReadIndicator"
		     + " group=" + group
		     + " article=" + n.anum
		     + classLoaderSrc
		     + ">\n");

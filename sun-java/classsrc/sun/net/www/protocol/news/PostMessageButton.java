/*
 * @(#)PostMessageButton.java	1.11 95/08/22 James Gosling, Jonathan Payne
 * 
...
class PostMessageButton extends Button implements Runnable {
	...
	if (user.length() == 0) {
	    ...
	    String ouser = System.getProperty("user.emailname");
	    SecurityManager.resetScopePermission();
	    if (!user.equals(ouser)) {
	        SecurityManager.setScopePermission();
		Properties p = System.getProperties();
		SecurityManager.resetScopePermission();
		p.put("user.emailname", user);
//REMIND	p.save();
	    }
	if (article.mailing) {
	    try {
		...
		SecurityManager.setScopePermission();
		ms.print("x-Mailer: " +
			 System.getProperty("programname",
					    System.getProperty("hotjava.title", "Java")) +

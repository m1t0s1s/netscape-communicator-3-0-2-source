 ...
 * All Rights Reserved.
 *
 * @(#)X11Graphics.java	1.35 95/12/04 11/14/94
 *
 *      Sami Shaio, 11/14/94
 */
 ...
 * particular canvas. 
 * 
 * @version 1.35 04 Dec 1995
 * @author Sami Shaio
 */

public class X11Graphics extends Graphics {
    int		pData;
    Color	foreground;
    Font	font;
    ...
    private native void createFromGraphics(X11Graphics g);
	...
	createFromGraphics(g);
        ...
        font = g.getFont();
        // An X11Graphics is always supposed to have a non-nil font, but check anyway.
        if( font == null ) {
           font = new Font("Dialog", Font.PLAIN, 12);
        }
        pSetFont( font );
    }
    X11Graphics(MComponentPeer comp) {
	createFromComponent(comp);
        
        font = comp.target.getFont();
        // A Component is always supposed to have a non-nil font, but check anyway
        if( font == null ) {
           font = new Font("Dialog", Font.PLAIN, 12);
        }
        pSetFont( font );
    ...
    public X11Graphics(X11Image image) {
	...
	X11Graphics g = new X11Graphics(this);
    ...
    /**
     * Disposes of this X11Graphics context. It cannot be used after being
     * disposed.
     */
	...
	ImageRepresentation ir = ximg.getImageRep(-1, -1);
	...
	ImageRepresentation ir = ximg.getImageRep(width, height);
	return ir.drawImage(this, x, y, null, observer);
    }

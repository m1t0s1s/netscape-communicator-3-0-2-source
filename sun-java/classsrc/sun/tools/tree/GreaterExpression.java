/*
 * @(#)GreaterExpression.java	1.11 95/09/14 Arthur van Hoff
 *
...
class GreaterExpression extends BinaryCompareExpression {
	...
	switch (left.type.getTypeCode()) {
	  case TC_INT:
		...
		asm.add(where, whenTrue ? opc_if_icmpgt : opc_if_icmple, lbl);
		return;
	    }
	    break;
	  case TC_LONG:
	...
	asm.add(where, whenTrue ? opc_ifgt : opc_ifle, lbl);
    }
}

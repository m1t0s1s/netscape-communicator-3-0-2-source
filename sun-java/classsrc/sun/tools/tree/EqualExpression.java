/*
 * @(#)EqualExpression.java	1.13 95/09/14 Arthur van Hoff
 *
...
class EqualExpression extends BinaryEqualityExpression {
	...
	switch (left.type.getTypeCode()) {
	  ...
	  case TC_INT:
		...
		asm.add(where, whenTrue ? opc_if_icmpeq : opc_if_icmpne, lbl);
		return;
	    }
	    break;
	  case TC_LONG:
	  ...
	  case TC_CLASS:
	    if (right.equals(0)) {
		asm.add(where, whenTrue ? opc_ifnull : opc_ifnonnull, lbl);
	    } else {
		right.codeValue(env, ctx, asm);
		asm.add(where, whenTrue ? opc_if_acmpeq : opc_if_acmpne, lbl);
	    }
	    return;
	...
	asm.add(where, whenTrue ? opc_ifeq : opc_ifne, lbl);
    }
}

/*
 * @(#)NotEqualExpression.java	1.13 95/09/14 Arthur van Hoff
 *
...
class NotEqualExpression extends BinaryEqualityExpression {
	...
	switch (left.type.getTypeCode()) {
	  ...
	  case TC_INT:
		...
		asm.add(where, whenTrue ? opc_if_icmpne : opc_if_icmpeq, lbl);
		return;
	    }
	    break;
	  case TC_LONG:
	  ...
	  case TC_CLASS:
	    if (right.equals(0)) {
		asm.add(where, whenTrue ? opc_ifnonnull : opc_ifnull, lbl);
	    } else {
		right.codeValue(env, ctx, asm);
		asm.add(where, whenTrue ? opc_if_acmpne : opc_if_acmpeq, lbl);
	    }
	    return;
	  default:
